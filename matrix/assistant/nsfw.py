...
@matrix.tgbot.on(CallbackQuery(data=re.compile(r"^age_verification_true")))
...
@matrix.tgbot.on(CallbackQuery(data=re.compile(r"^age_verification_false")))
...
@matrix.tgbot.on(CallbackQuery(data=re.compile(r"^chg_of_decision_")))
async def chg_of_decision_(event: CallbackQuery):
    ...
    try:
        await event.edit(
            ...
            file="https://graph.org/file/c864b32fa1a8075317309.jpg",
            buttons=buttons,
        )
    except MessageNotModifiedError:
        pass
