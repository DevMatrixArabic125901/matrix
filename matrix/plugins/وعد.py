...
@matrix.ar_cmd(pattern="تجميع$")
async def _(event):
    await event.edit("حسنا, تأكد من انك مشترك ب قنوات الاشتراك الاجباري لتجنب الأخطأء")
    channel_entity = await matrix.get_entity("@@zmmbot")
    await matrix.send_message("@@zmmbot", "/start")
    await asyncio.sleep(5)
    msg0 = await matrix.get_messages("@@zmmbot", limit=1)
    await msg0[0].click(2)
    await asyncio.sleep(5)
    msg1 = await matrix.get_messages("@@zmmbot", limit=1)
    ...
    for i in range(100):
        ...
        try:
            ...
            msg2 = await matrix.get_messages("@@zmmbot", limit=1)
...
@matrix.ar_cmd(pattern="بخشيش وعد (.*)")
...
@matrix.ar_cmd(pattern="راتب وعد (.*)")
...
# none
@matrix.ar_cmd(pattern="كلمات وعد (.*)")
