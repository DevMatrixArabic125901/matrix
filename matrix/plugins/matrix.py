...
async def matrixteam(matrixevent):
    ...
    if vname in vlist:
        ...
        if cmd == "اضف":
            if not vinfo and vname == "ALIVE_TEMPLATE":
                return await edit_delete(event, f"**᥀ ︙ 📑 يرجى متابع قناه الفارات تجدها هنا : @matrix")
...
@matrix.on(admin_cmd(pattern="رفع كحبه(?:\s|$)([\s\S]*)"))
async def ma(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
       return
...
@matrix.on(admin_cmd(pattern="رفع كواد(?:\s|$)([\s\S]*)"))
async def ma(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
    ...
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**᥀ ¦ المستخدم ⪼ • ** [{mat2}](tg://user?id={user.id}) \n ☑️ **¦  تم رفعه كواد ينيج 🍌. ** \n**🍚 ¦ بواسطه  :** {my_mention} ")
@matrix.on(admin_cmd(pattern="رفع مرتبط(?:\s|$)([\s\S]*)"))
async def ma(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
    ...
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**᥀ ¦ المستخدم ⪼ • ** [{mat2}](tg://user?id={user.id}) \n ☑️ **¦ تم رفعه مرتبط  👩‍❤️‍💋‍👨. .** \n**🍚 ¦ بواسطه  :** {my_mention} ")
@matrix.on(admin_cmd(pattern="رفع مثلي(?:\s|$)([\s\S]*)"))
async def ma(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
