...
kno = [    "100% تحبك وتخاف عليك",    "100% يحبج ويخاف عليج",    "91% جـزء من گـلبه ",    "81% تموت عليك ههاي ",    "81% يموت عليج ههذا ",    "هاه اخي ؟  🏳‍🌈",    "40% واحد حيوان ومصلحه عوفه ",    "50% شوف شعندك وياه ",    "30% خاين نصحيا عوفيه ميفيدج ",    "25% مصادق غيرج ويكلج احبج",    "25% واحد كلب ابن كلب عوفه",    "0% يكهرك ",    "0% تكرهك ",    "@matrix",]
...
async def matrext(event):
    ismaknoma = gvarstatus("ismamailma")
    if not ismaknoma:
...
@matrix.on(admin_cmd(pattern="رفع فرخ(?:\s|$)([\s\S]*)"))
async def ma(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
async def ma(mention):
    ...
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**🚹 ¦ المستخدم ⪼ • ** [{mat2}](tg://user?id={user.id}) \n ☑️ **¦  تـم رفـعه غـبي 🛏️ .** \n**🤵‍♂️ ¦ بواسطه  : ** {my_mention} ")
@matrix.on(admin_cmd(pattern="رفع سعلوه(?:\s|$)([\s\S]*)"))
async def ma(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
