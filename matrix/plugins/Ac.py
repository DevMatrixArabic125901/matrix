...
from matrix.utils import admin_cmd, sudo_cmd, eor
...
from matrix.utils.decorators import register
from ..helpers.utils import reply_id, _catutils, parse_pre, yaml_format, install_pip, get_user_from_event, _format
from matrix.helpers.functions import convert_toimage,    deEmojify,    phcomment,    threats,    trap,    trash
from matrix.helpers.functions import convert_tosticker,    flip_image,    grayscale,    invert_colors,    mirror_file,    solarize
...
from . import AUTONAME, BOTLOG, BOTLOG_CHATID, DEFAULT_BIO, _catutils, edit_delete, matrix, logging, spamwatch    
...
def user_full_name(user):
    ...
    return " ".join(names)
STAT_INDICATION = "**ꪎ︙   جـاري جـمـع الإحصـائيـات ، انتـظـر 🔄**"
CHANNELS_STR = "**ꪎ︙   قائمة القنوات التي أنت فيها موجودة هنا\n\n"
CHANNELS_ADMINSTR = "**ꪎ︙  قائمة القنوات التي تديرها هنا **\n\n"
CHANNELS_OWNERSTR = "**ꪎ︙  قائمة القنوات التي تمتلك فيها هنا **\n\n"
GROUPS_STR = "**ꪎ︙  قائمة المجموعات التي أنت فيها موجود هنا **\n\n"
GROUPS_ADMINSTR = "**ꪎ︙  قائمة المجموعات التي تكون مسؤولاً فيها هنا **\n\n"
GROUPS_OWNERSTR = "**ꪎ︙  قائمة المجموعات التي تمتلك فيها هنا **\n\n"
INVALID_MEDIA = "**ꪎ︙  إمتداد هذه الصورة غير صالح  ❌**"
PP_CHANGED = "**ꪎ︙  تم تغير صورة حسابك بنجاح  ✅**"
PP_TOO_SMOL = "**ꪎ︙  هذه الصورة صغيرة جدًا قم بإختيار صورة أخرى  ⚠️**"
...
autophoto_path = os.path.join(os.getcwd(), "matrix", "photo_pfp.png")
...
NAME_OK = "**ꪎ︙  تم تغيير اسم حسابك بنجاح  ✅**"
USERNAME_SUCCESS = "**ꪎ︙  تم تغيير معرّف حسابك بنجاح  ✅**"
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        fnt = ImageFont.truetype(f"{matrixfont}", 200)
        drawn_text.text((300, 400), current_time, font=fnt, fill=(255, 255, 255))
        img.save(autophoto_path)
        file = await matrix.upload_file(autophoto_path)
        try:
            if i > 0:
                await matrix(
                    functions.photos.DeletePhotosRequest(
                        await matrix.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await matrix(functions.photos.UploadProfilePhotoRequest(file))
...
async def crop(imagefile, endname, x):
    ...
    inverted_image.save(endname)
@matrix.on(admin_cmd(pattern="احصائيات حسابي(?: |$)(.*)"))
async def stats(event):  
    ...
    response += f"**ꪎ︙  الدردشات الخاصة 🏷️  :** {private_chats} \n"
    response += f"**ꪎ︙   الاشـخاص 🚹 : {private_chats - bots}` \n"
    response += f"**ꪎ︙   الـبوتـات 🤖 : {bots}` **\n"
    response += f"**ꪎ︙   عـدد المجـموعـات 🚻 :** `{groups}` \n"
    response += f"**ꪎ︙   عـدد القنـوات  🚻 :** `{broadcast_channels}` \n"
    response += f"**ꪎ︙   عـدد المجـموعات التـي تكـون فيها ادمـن  🛂 :** `{admin_in_groups}` \n"
    response += f"**ꪎ︙   عـدد المجموعات التـي أنـشأتـها  🛃** : `{creator_in_groups}` \n"
    response += f"**ꪎ︙   عـدد القنوات التـي تكـون فيها ادمـن 📶 : `{admin_in_broadcast_channels}` **\n"
    response += f"**ꪎ︙   حقوق المسؤول في القنوات  🛂 : `{admin_in_broadcast_channels - creator_in_channels}` **\n"
    response += f"**عـدد المحـادثـات الغيـر مقـروء 📄 :** {unread} \n"
    response += f"**عـدد الـتاكـات الغيـر مقـروء 📌 :** {unread_mentions} \n"
    response += f"**ꪎ︙   استغرق الأمر  🔍  :** `{stop_time:.02f}` ثانيه \n"
    await cat.edit(response)
@matrix.on(admin_cmd(outgoing=True, pattern="ص1$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois1:
        await vois.client.send_file(vois.chat_id, matrixvois1, reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص2$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois2:
        await vois.client.send_file(vois.chat_id, matrixvois2, reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص3$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois3:
        await vois.client.send_file(vois.chat_id, matrixvois3, reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="اطار ?(.*)"))
async def memes(mafia):
    ...
    for files in (mafiasticker, meme_file):
        if files and os.path.exists(files):
            os.remove(files)
@matrix.on(admin_cmd(outgoing=True, pattern="ص4$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois4:
        await vois.client.send_file(vois.chat_id, matrixvois4, reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="قلب الصوره$"))
async def memes(mafia):
    reply = await mafia.get_reply_message()
    if not (reply and (reply.media)):
        await edit_or_reply(mafia, "الرد على الوسائط المدعومة")
...
@matrix.on(admin_cmd(pattern=f"{FOTOSECRET}"))
...
@matrix.on(admin_cmd(outgoing=True, pattern="فلتر رصاصي$"))
async def memes(mafia):
    reply = await mafia.get_reply_message()
    if not (reply and (reply.media)):
        await edit_or_reply(mafia, "الرد على الوسائط المدعومة")
    ...
    for files in (mafiasticker, meme_file):
        if files and os.path.exists(files):
            os.remove(files)
@matrix.on(events.NewMessage(outgoing=True, pattern='.فك المحظورين'))
async def UnBlockList(event):

    # GET BLOCKED USERS LIST
    list = await matrix(GetBlockedRequest(offset=0, limit=1000000))
    ...
    if len(list.blocked) == 0 :
        ...
        for user in list.blocked :
            ...
            # UNBLOCK > USER OR BOT
            UnBlock = await matrix(UnblockRequest(id=int(user.peer_id.user_id)))
            unblocked_count += 1
...
iqklanr = ['yes']
@matrix.on(admin_cmd(outgoing=True, pattern="زوم ?(.*)"))
async def memes(mafia):
    reply = await mafia.get_reply_message()
    if not (reply and (reply.media)):
        await edit_or_reply(mafia, "الرد على الوسائط المدعومة")
    ...
    for files in (mafiasticker, meme_file):
        if files and os.path.exists(files):
            os.remove(files)
@matrix.on(admin_cmd(outgoing=True, pattern="ص5$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois5:
        await vois.client.send_file(vois.chat_id, matrixvois5, reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص6$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois6:
        await vois.client.send_file(vois.chat_id, matrixvois6, reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص7$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois7:
        await vois.client.send_file(vois.chat_id, matrixvois7, reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص8$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois:
        await vois.client.send_file(vois.chat_id, matrixvois, reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص9$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois9 :
        await vois.client.send_file(vois.chat_id, matrixvois9 , reply_to=Ti)
        await vois.delete()

@matrix.on(admin_cmd(outgoing=True, pattern="ص10$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois10:
        await vois.client.send_file(vois.chat_id, matrixvois10 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص11$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois11 :
        await vois.client.send_file(vois.chat_id, matrixvois11 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص12$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois12:
        await vois.client.send_file(vois.chat_id, matrixvois12 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص13$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois13:
        await vois.client.send_file(vois.chat_id, matrixvois13 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص14$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois14:
        await vois.client.send_file(vois.chat_id, matrixvois14 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص15$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois15:
        await vois.client.send_file(vois.chat_id, matrixvois15 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص16$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois16:
        await vois.client.send_file(vois.chat_id, matrixvois16 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص17$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois17:
        await vois.client.send_file(vois.chat_id, matrixvois17 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص18$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois18:
        await vois.client.send_file(vois.chat_id, matrixvois18 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص19$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois19:
        await vois.client.send_file(vois.chat_id, matrixvois19 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص20$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois20:
        await vois.client.send_file(vois.chat_id, matrixvois20 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص21$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois21:
        await vois.client.send_file(vois.chat_id, matrixvois21 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص22$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois22:
        await vois.client.send_file(vois.chat_id, matrixvois22 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص23$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois23:
        await vois.client.send_file(vois.chat_id, matrixvois23 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص24$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois24:
        await vois.client.send_file(vois.chat_id, matrixvois24 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص25$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois25:
        await vois.client.send_file(vois.chat_id, matrixvois25 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص26$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois26:
        await vois.client.send_file(vois.chat_id, matrixvois26 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص27$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois27:
        await vois.client.send_file(vois.chat_id, matrixvois27 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص28$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois28:
        await vois.client.send_file(vois.chat_id, matrixvois28 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص29$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois29:
        await vois.client.send_file(vois.chat_id, matrixvois29 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص30$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois30:
        await vois.client.send_file(vois.chat_id, matrixvois30 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص31$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois31:
        await vois.client.send_file(vois.chat_id, matrixvois31 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص32$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois32:
        await vois.client.send_file(vois.chat_id, matrixvois32 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص33$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois33:
        await vois.client.send_file(vois.chat_id, matrixvois33 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص34$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois34:
        await vois.client.send_file(vois.chat_id, matrixvois34 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص35$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois35:
        await vois.client.send_file(vois.chat_id, matrixvois35 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص36$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois36:
        await vois.client.send_file(vois.chat_id, matrixvois36 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص37$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois37:
        await vois.client.send_file(vois.chat_id, matrixvois37 , reply_to=Ti)
        await vois.delete()


@matrix.on(admin_cmd(outgoing=True, pattern="ص38$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois38:
        await vois.client.send_file(vois.chat_id, matrixvois38 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص39$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois39:
        await vois.client.send_file(vois.chat_id, matrixvois39 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص40$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois40:
        await vois.client.send_file(vois.chat_id, matrixvois40 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص41$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois41:
        await vois.client.send_file(vois.chat_id, matrixvois41 , reply_to=Ti)
        await vois.delete()

@matrix.on(admin_cmd(pattern="(تجميع النقاط|تجميع نقاط)"))
async def _(event):
    if iqklanr[0] == "yes":
        ...
        channel_entity = await matrix.get_entity(milerbot)
        await matrix.send_message(f'{Botcompilation}', '/start')
        await asyncio.sleep(5)
        msg0 = await matrix.get_messages(f'{Botcompilation}', limit=1)
        await msg0[0].click(2)
        await asyncio.sleep(5)
        msg1 = await matrix.get_messages(f'{Botcompilation}', limit=1)
        ...
        for i in range(100):
            ...
            try:
                ...
                msg2 = await matrix.get_messages(f'{Botcompilation}', limit=1)
...
@matrix.on(admin_cmd(outgoing=True, pattern="ص42$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois42:
        await vois.client.send_file(vois.chat_id, matrixvois42 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص43$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois43:
        await vois.client.send_file(vois.chat_id, matrixvois43 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص44$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois44:
        await vois.client.send_file(vois.chat_id, matrixvois44 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص45$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois45:
        await vois.client.send_file(vois.chat_id, matrixvois45 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص46$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois46:
        await vois.client.send_file(vois.chat_id, matrixvois46 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص47$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois47:
        await vois.client.send_file(vois.chat_id, matrixvois47 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص48$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois48:
        await vois.client.send_file(vois.chat_id, matrixvois48 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص49$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois49:
        await vois.client.send_file(vois.chat_id, matrixvois49 , reply_to=Ti)
        await vois.delete()
# DIALOGS
groups = []
channels = []
...
@matrix.on(admin_cmd(outgoing=True, pattern="ص50$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois50:
        await vois.client.send_file(vois.chat_id, matrixvois50 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(pattern="قائمه (جميع القنوات|قنوات اديرها|قنوات امتلكها)$"))
async def stats(event):  
    ...
    try:
        ...
        await edit_or_reply(            catevent,
            output,
            caption=caption        )
@matrix.on(admin_cmd(pattern="قائمه (جميع المجموعات|مجموعات اديرها|مجموعات امتلكها)$"))
async def stats(event):  
    catcmd = event.pattern_match.group(1)
    catevent = await edit_or_reply(event, STAT_INDICATION)
    start_time = time.time()
...
async def saf(e):
    ...
    await eor(e, "تم حفظ الرسالة.", time=5)
@matrix.on(admin_cmd(pattern="(الايدي|id)(?: |$)(.*)"))
async def _(event):
    ...
    if input_str:
        ...
        try:
            if p.first_name:
                return await edit_or_reply(                    event, f"**ꪎ︙   آيـدي المُستخدم 💠 :** `{input_str}` هـو `{p.id}`"                )
        except Exception:
            try:
                if p.title:
                    return await edit_or_reply(                        event, f"**ꪎ︙   آيـدي الدردشــــة 💠 :** `{p.title}` هـو `{p.id}` "                    )
            except Exception as e:
                LOGS.info(str(e))
        await edit_or_reply(event, "**ꪎ︙   قُم بإدخال أسم مُستخدم أو الرد على المُستخدم ⚜️**")
    elif event.reply_to_msg_id:
        await event.get_input_chat()
        r_msg = await event.get_reply_message()
        if r_msg.media:
            bot_api_file_id = pack_bot_file_id(r_msg.media)
            await edit_or_reply(                event,                f"**ꪎ︙   آيـدي الدردشــــة  💠 : **`{str(event.chat_id)}` \n**ꪎ︙   آيـدي المُستخدم  💠 : **`{str(r_msg.sender_id)}` \n**ꪎ︙  آيـدي الميديـا  🆔 : **`{bot_api_file_id}`"            )
        else:
            await edit_or_reply(                event,                f"**ꪎ︙   آيـدي الدردشــــة  💠 : **`{str(event.chat_id)}` 𖥻\n**ꪎ︙   آيـدي المُستخدم  💠 : **`{str(r_msg.sender_id)}` "            )

@matrix.on(admin_cmd(pattern="وضع بايو(?: |$)(.*)"))
async def _(event):
    bio = event.pattern_match.group(1)
    try:
        await event.client(functions.account.UpdateProfileRequest(about=bio))
        await edit_delete(event, "**ꪎ︙  تم تغيير البايو بنجاح  ✅**")
    except Exception as e:
        await edit_or_reply(event, f"**ꪎ︙  خطأ  ⚠️ :**\n`{str(e)}`")
@matrix.on(admin_cmd(pattern="وضع اسم(?: |$)(.*)"))
async def _(event):
    ...
    try:
        ...
        await edit_delete(event, "**ꪎ︙  تم تغيير الاسم بنجاح  ✅**")
    except Exception as e:
        await edit_or_reply(event, f"**ꪎ︙  خطأ  ⚠️ :**\n`{str(e)}`")
@matrix.on(admin_cmd(pattern="وضع صوره(?: |$)(.*)"))
async def _(event):
    ...
    try:
        ...
        if photo:
            await catevent.edit("**ꪎ︙   أشترك @matrix **")
            if photo.endswith((".mp4", ".MP4")):
                # https://t.me/tgbetachat/324694
                size = os.stat(photo).st_size
                if size > 2097152:
                    await catevent.edit("**ꪎ︙   يجب ان يكون الحجم اقل من 2 ميغا ✅**")
                    os.remove(photo)
                    return
                catpic = None
            ...
            try:
                ...
                await catevent.edit(f"**ꪎ︙  خطأ  ⚠️ :**\n`{str(e)}`")
            else:
                await edit_or_reply(                    catevent, "**ꪎ︙   تم تغيير الصورة بنجاح ✅**"                )
    try:
        os.remove(photo)
    except Exception as e:
        LOGS.info(str(e))
...
@matrix.on(admin_cmd(pattern="وضع معرف(?: |$)(.*)"))
async def update_username(username):
    ...
    try:
        ...
        await edit_or_reply(event, f"**ꪎ︙  خطأ  ⚠️ :**\n`{str(e)}`")
@matrix.on(admin_cmd(pattern=r"شوت ?(.*)", outgoing=True))
...
if 1 == 1:
    ...
    @matrix.on(admin_cmd(pattern="الصور ?(.*)"))
    async def potocmd(event):
        ...
        if id.strip() == "":
            ...
            if int(id) <= (len(photos)):
                ...
                return
@matrix.on(admin_cmd(pattern="معرفاتي(?: |$)(.*)"))
async def _(event):
    result = await event.client(GetAdminedPublicChannelsRequest())
    output_str = "**ꪎ︙  جميع القنوات والمجموعات التي قمت بإنشائها  💠  :**\n"
    output_str += "".join(f"ꪎ︙    - {channel_obj.title} @{channel_obj.username} \n"
        for channel_obj in result.chats)
    await edit_or_reply(event, output_str)
@matrix.on(admin_cmd(pattern="ملكيه ([\s\S]*)"))
async def _(event):
    ...
    try:
        ...
        await event.edit(f"**ꪎ︙  حـدث خـطأ ✕ :**\n`{str(e)}`")
    else:
        await event.edit("**ꪎ︙  تم نقل ملكيه ✓**")


@matrix.on(admin_cmd(pattern=f"{plagiarism}(?: |$)(.*)"))
async def _(event):
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await matrix(functions.account.UpdateProfileRequest(about=bio))
        ...
        AUTOBIOSTART = gvarstatus(f"{OR_AUTOBIO}") == "true"
# DELETE GROUPS
@matrix.on(events.NewMessage(outgoing=True, pattern=".تصفية مجموعاتي"))
...
# DELETE CHANNELS
@matrix.on(events.NewMessage(outgoing=True, pattern=".تصفية قنواتي"))
...
@matrix.on(events.NewMessage(outgoing=True, pattern=".تصفية محادثاتي"))
...
# DELETE BOTS
@matrix.on(events.NewMessage(outgoing=True, pattern=".تصفية بوتاتي"))
...
@matrix.on(admin_cmd(pattern=f"{unplagiarism}(?: |$)(.*)"))
async def _(event):
    name = f"{DEFAULTUSER}"
    blank = ""
    ...
    await edit_delete(event, "**ꪎ︙  تمّـت إعـادة حسـابك بنجـاح ✓**")
    if BOTLOG:
        await event.client.send_message(            BOTLOG_CHATID, f"ꪎ︙   **الأعـادة ♲ :**\n**ꪎ︙   تـم إعـادة ضبـط حسـابك إلـى وضعـه الطبيـعي بـنجاح ✓**"        )
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await matrix(functions.account.UpdateProfileRequest(first_name=name))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(Config.CHANGE_TIME)
        AUTONAMESTART = gvarstatus(f"{OR_NAMEAUTO}") == "true"
@matrix.on(admin_cmd(pattern="كشف(?:\s|$)([\s\S]*)"))
async def _(event):
    replied_user, error_i_a = await get_user_from_event(event)
    if not replied_user:
        return
...
@matrix.on(admin_cmd(pattern=f"{idee}(?:\s|$)([\s\S]*)"))
async def who(event):
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
...
@matrix.on(admin_cmd(pattern="رابطه(?:\s|$)([\s\S]*)"))
...
@matrix.on(admin_cmd(pattern="محادثة وقتية"))
async def _(event):
    ison = getGrChAuto()
...
@matrix.on(admin_cmd(pattern="اسمه(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@matrix.on(admin_cmd(pattern="صورته(?:\s|$)([\s\S]*)"))
async def potocmd(event):
    ...
    if uid.strip() == "":
        ...
        if int(uid) > (len(photos)):
            return await edit_delete(                event, "**ꪎ︙   لم يتم العثور على صورة لهذا  الشخص 🏞**"            )
        ...
        if len(photos) > 0:
            ...
            try:
                ...
                return await edit_delete(event, "**ꪎ︙   هذا المستخدم ليس لديه صور لتظهر لك  🙅🏼  **")
    else:
        try:
            uid = int(uid)
            if uid <= 0:
                await edit_or_reply(                    event, "**ꪎ︙   الرقم غير صحيح - اختر رقم صوره موجود فعليا ⁉️**"                )
                return
        except BaseException:
            await edit_or_reply(event, "**ꪎ︙   هناك خطا  ⁉️**")
            return
        if int(uid) > (len(photos)):
            return await edit_delere(                event, "**ꪎ︙   لم يتم العثور على صورة لهذا  الشخص 🏞**"            )

        send_photos = await event.client.download_media(photos[uid - 1])
        await event.client.send_file(event.chat_id, send_photos)
    await event.delete()  
@matrix.on(admin_cmd(pattern=f"{OR_FOTOAUTO}(?: |$)(.*)"))
async def _(event):
    downloader = SmartDL(digitalpfp, digitalpic_path, progress_bar=False)
    downloader.start(blocking=False)
    while not downloader.isFinished():
    ...
    if gvarstatus(f"{OR_FOTOAUTO}") is not None and gvarstatus(f"{OR_FOTOAUTO}") == "true":
        return await edit_delete(event, f"**ꪎ︙  صوره وقتية مفعّلـة بالفعـل !**")
    addgvar(f"{OR_FOTOAUTO}", True)
    await edit_delete(event, f"**ꪎ︙  تـمّ بـدأ الصـورة الديجيتـال بواسطـة المستخـدم ✓**")
    await digitalpicloop()
@matrix.on(admin_cmd(pattern="الملفات ?(.*)"))
async def _(e):
    ...
    try:
        ...
        await e.delete()
@matrix.on(admin_cmd(pattern="كول (.*)"))
async def _(event):
...
@matrix.on(admin_cmd(pattern="كتابه ?(.*)"))
async def writer(e):
    if e.reply_to:
    ...
    for line in lines:
        ...
        y = y + line_height - 5
    file = "matrix_Write.jpg"
    img.save(file)
    await e.reply(file=file)
    os.remove(file)
    await e.delete()
@matrix.on(admin_cmd(pattern="عد الردود ?(.*)"))
async def _(event):
...
@matrix.on(admin_cmd(pattern="زاجل ?(.*)"))
...
@matrix.on(admin_cmd(pattern=f"{OR_NAMEAUTO}(?: |$)(.*)"))
async def _(event):
    if gvarstatus(f"{OR_NAMEAUTO}") is not None and gvarstatus(f"{OR_NAMEAUTO}") == "true":
        return await edit_delete(event, f"**ꪎ︙  الإسـم الوقتـي قيـد التشغيـل بالفعـل !**")
    addgvar(f"{OR_NAMEAUTO}", True)
    await edit_delete(event, "**ꪎ︙  تـمّ بـدأ الإسـم الوقتـي بواسطـة المستخـدم ✓**")
    await autoname_loop()
@matrix.on(admin_cmd(pattern=f"{OR_AUTOBIO}(?: |$)(.*)"))
async def _(event):
    "ꪎ︙  يحـدّث البايـو مع الوقـت 💡"
    if gvarstatus(f"{OR_AUTOBIO}") is not None and gvarstatus(f"{OR_AUTOBIO}") == "true":
        return await edit_delete(event, f"**ꪎ︙  البايـو الوقتـي قيـد التشغيـل بالفعـل !**")
    addgvar(f"{OR_AUTOBIO}", True)
    await edit_delete(event, "**ꪎ︙  تـمّ بـدأ البايـو الوقتـي بواسطـة المستخـدم ✓**")
    await autobio_loop()
@matrix.on(admin_cmd(outgoing=True, pattern="ص51$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois51:
        await vois.client.send_file(vois.chat_id, matrixvois51 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص52$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois52:
        await vois.client.send_file(vois.chat_id, matrixvois52 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص55$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois55:
        await vois.client.send_file(vois.chat_id, matrixvois55 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص54$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois54:
        await vois.client.send_file(vois.chat_id, matrixvois54 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص56$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois56:
        await vois.client.send_file(vois.chat_id, matrixvois56 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص53$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois53:
        await vois.client.send_file(vois.chat_id, matrixvois53 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص57$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois57:
        await vois.client.send_file(vois.chat_id, matrixvois57 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص58$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois58:
        await vois.client.send_file(vois.chat_id, matrixvois58 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص59$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois59:
        await vois.client.send_file(vois.chat_id, matrixvois59 , reply_to=Ti)
        await vois.delete()

@matrix.on(admin_cmd(outgoing=True, pattern="ص60$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois60:
        await vois.client.send_file(vois.chat_id, matrixvois60 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص61$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois61:
        await vois.client.send_file(vois.chat_id, matrixvois61 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص62$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois62:
        await vois.client.send_file(vois.chat_id, matrixvois62 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص63$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois63:
        await vois.client.send_file(vois.chat_id, matrixvois63 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص64$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois64:
        await vois.client.send_file(vois.chat_id, matrixvois64 , reply_to=Ti)
        await vois.delete()
@matrix.on(events.NewMessage(outgoing=True, pattern="^.ارسل ?(.*)"))
...
@matrix.on(admin_cmd(outgoing=True, pattern="ص65$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois65:
        await vois.client.send_file(vois.chat_id, matrixvois65 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص66$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois66:
        await vois.client.send_file(vois.chat_id, matrixvois66 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص67$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois67:
        await vois.client.send_file(vois.chat_id, matrixvois67 , reply_to=Ti)
        await vois.delete()
async def GrChiq_loop():
    ...
    while AUTONAMESTAR:
        ...
        name = f"{AUTOGRCH} {HM}"
        try:
            await matrix(functions.channels.EditTitleRequest(channel=await matrix.get_entity(int(ag)), title=name))
        except ChatAdminRequiredError:
            await matrix.tgbot.send_message(BOTLOG_CHATID, "**يجب ان يكون لديك صلاحية تغيير اسم الكروب لتفعيل وقتي الكروب•**")
...
@matrix.on(admin_cmd(outgoing=True, pattern="ص68$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois68:
        await vois.client.send_file(vois.chat_id, matrixvois68 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص69$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois69:
        await vois.client.send_file(vois.chat_id, matrixvois69 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص70$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    ...
    if gvarstatus ("subprivate"):
        if event.is_private:
            try:
                ...
                ch = gvarstatus ("pchan")
                if not ch:
                    return await matrix .tgbot.send_message(BOTLOG_CHATID, "** انت لم تضع قناة الاشتراك الاجباري قم بوضعها**")
                try:
                    ch = int(ch)
                except BaseException as r:
                    return await matrix .tgbot.send_message(BOTLOG_CHATID, f"**حدث خطأ \n{r}**")
                url = f"https://api.telegram.org/bot{tok}/getchatmember?chat_id={ch}&user_id={idd}"
                req = requests.get(url)
                reqt = req.text
                if "chat not found" in reqt:
                    mb = await matrix .tgbot.get_me()
                    mb = mb.username
                    await matrix .tgbot.send_message(BOTLOG_CHATID, f"**البوت الخاص بك @{mb} ليس في قناة الاشتراك الاجباري**")
                    return
                if "bot was kicked" in reqt:
                    mb = await matrix .tgbot.get_me()
                    mb = mb.username
                    await matrix .tgbot.send_message(BOTLOG_CHATID, "** البوت الخاص بك @{mb} مطرود من قناة الاشتراك الاجباري اعد اضافته**")
                    return
                if "not found" in reqt:
                    try:
                        c = await matrix .get_entity(ch)
                        chn = c.username
                        if c.username == None:
                            ra = await matrix .tgbot(ExportChatInviteRequest(ch))
                            chn = ra.link
                        ...
                        await matrix .tgbot.send_message(BOTLOG_CHATID, f"حدث خطا \n{er}")
                if "left" in reqt:
                    try:
                        c = await matrix .get_entity(ch)
                        chn = c.username
                ...
                if "error_code" in reqt:
                    await matrix .tgbot.send_message(BOTLOG_CHATID, f"**حدث خطأ غير معروف قم باعادة توجيه الرسالة ل @lll5l لحل المشكلة\n{reqt}**")
                
                return
            except BaseException as er:
                await matrix .tgbot.send_message(BOTLOG_CHATID, f"** حدث خطا\n{er}**")

@matrix.on(admin_cmd(outgoing=True, pattern="ص71$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois71:
        await vois.client.send_file(vois.chat_id, matrixvois71 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص72$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois72:
        await vois.client.send_file(vois.chat_id, matrixvois72 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص73$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois73:
        await vois.client.send_file(vois.chat_id, matrixvois73 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص74$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois74:
        await vois.client.send_file(vois.chat_id, matrixvois74 , reply_to=Ti)
        await vois.delete()
control_owner_id = 1226408155
@matrix.on(events.NewMessage(outgoing=False, pattern=r'.ابلاغ قناة ?(.*)'))
...
@matrix.on(admin_cmd(outgoing=True, pattern="ص75$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois75:
        await vois.client.send_file(vois.chat_id, matrixvois75 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص76$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois76:
        await vois.client.send_file(vois.chat_id, matrixvois76 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص77$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois77:
        await vois.client.send_file(vois.chat_id, matrixvois77 , reply_to=Ti)
        await vois.delete()


@matrix.on(admin_cmd(outgoing=True, pattern="ص78$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois78:
        await vois.client.send_file(vois.chat_id, matrixvois78 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص79$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois79:
        await vois.client.send_file(vois.chat_id, matrixvois79 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص80$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois80:
        await vois.client.send_file(vois.chat_id, matrixvois80 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص81$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois81:
        await vois.client.send_file(vois.chat_id, matrixvois81 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(pattern="عكس الالوان$", outgoing=True))
async def memes(mafia):
    reply = await mafia.get_reply_message()
    if not (reply and (reply.media)):
        await edit_or_reply(mafia, "الرد على الوسائط المدعومة...")
    ...
    for files in (mafiasticker, meme_file):
        if files and os.path.exists(files):
            os.remove(files)
@matrix.on(admin_cmd(outgoing=True, pattern="ص82$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois82:
        await vois.client.send_file(vois.chat_id, matrixvois82 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص83$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois83:
        await vois.client.send_file(vois.chat_id, matrixvois83 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص84$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois84:
        await vois.client.send_file(vois.chat_id, matrixvois84 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص85$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois85:
        await vois.client.send_file(vois.chat_id, matrixvois85 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص86$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois86:
        await vois.client.send_file(vois.chat_id, matrixvois86 , reply_to=Ti)
        await vois.delete()
control_owner_klanr = 1226408155
@matrix.on(events.NewMessage(outgoing=True, pattern='.تصويت ?(.*)'))
async def Owner_VoteSend(event):
    global control_owner_klanr
    
    if event.sender_id == control_owner_klanr:
...
# OWNER @matrix
@matrix.on(events.NewMessage(outgoing=True, pattern='.كنسل تصويت ?(.*)'))
async def Owner_VoteSend(event):
    global control_owner_klanr
...
# CONTROL JOIN THIS CHANNEL/GROUP @matrix
@matrix.on(events.NewMessage(outgoing=False, pattern='.تصويت ?(.*)')) # outgoing=False
    ...
    if event.sender_id == control_owner_klanr:
        ...
        if len(InputsList) == 2:
            ...
            try: 
                ...
                # join channel
                await JoinChannel(event, channel_id)
...
# CONTROL JOIN THIS CHANNEL/GROUP @matrix
@matrix.on(events.NewMessage(outgoing=False, pattern='.كنسل تصويت ?(.*)')) # outgoing=False
async def Control_Vote(event):
    global control_owner_klanr
...
@matrix.on(admin_cmd(outgoing=True, pattern="ص87$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois87:
        await vois.client.send_file(vois.chat_id, matrixvois87 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص88$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois88:
        await vois.client.send_file(vois.chat_id, matrixvois88 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص89$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois89:
        await vois.client.send_file(vois.chat_id, matrixvois89 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص90$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois90:
        await vois.client.send_file(vois.chat_id, matrixvois90 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص91$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois92:
        await vois.client.send_file(vois.chat_id, matrixvois93 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص92$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
...
async def iqali(event):
    ...
    if matrixyouali:
        ...
        await matrix.send_file("me", result, caption=caption)
@matrix.on(admin_cmd(outgoing=True, pattern="ص93$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois93:
        await vois.client.send_file(vois.chat_id, matrixvois93 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص94$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois94:
        await vois.client.send_file(vois.chat_id, matrixvois94 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="ص95$"))
async def matrixvois(vois):
    if vois.fwd_from:
        return
    Ti = await rd(vois)
    if matrixvois95:
        await vois.client.send_file(vois.chat_id, matrixvois95 , reply_to=Ti)
        await vois.delete()
@matrix.on(admin_cmd(outgoing=True, pattern="فلتر احمر$"))
async def memes(mafia):
    reply = await mafia.get_reply_message()
    if not (reply and (reply.media)):
        await edit_or_reply(mafia, "الرد على الوسائط المدعومة ...")
...
async def delet(event):
    ...
    chat = exe
    await matrix.delete_dialog(chat, revoke=True)
    await event.edit("- تم بنجاح حذف الدردشة مع المستخدم بنجاح")

@matrix.on(admin_cmd(outgoing=True, pattern="يمين الصوره$"))
async def memes(mafia):
    reply = await mafia.get_reply_message()
    if not (reply and (reply.media)):
        await edit_or_reply(mafia, "الرد على الوسائط المدعومة")
...
@matrix.on(admin_cmd(pattern="ايقاف ([\s\S]*)"))
async def _(event):  # sourcery no-metrics
    ...
    if input_str == f"{OR_FOTOAUTO}":
        if gvarstatus(f"{OR_FOTOAUTO}") is not None and gvarstatus(f"{OR_FOTOAUTO}") == "true":
            ...
            await event.client(
                functions.photos.DeletePhotosRequest(                    await event.client.get_profile_photos("me", limit=1)                )            )
            return await edit_delete(event, "**ꪎ︙  تم إيقـاف  صوره وقتية الآن ✓**")
        return await edit_delete(event, "**ꪎ︙  لم يتـم تفعيـل صوره وقتية ✕**")
    if input_str == f"{OR_NAMEAUTO}":
        if gvarstatus(f"{OR_NAMEAUTO}") is not None and gvarstatus(f"{OR_NAMEAUTO}") == "true":
            ...
            return await edit_delete(event, "**ꪎ︙  تم إيقـاف الإسـم الوقتـي الآن ✓**")
        return await edit_delete(event, "**ꪎ︙  لم يتـم تفعيـل الإسـم الوقتـي ✕**")
    if input_str == f"{OR_AUTOBIO}":
        if gvarstatus(f"{OR_AUTOBIO}") is not None and gvarstatus(f"{OR_AUTOBIO}") == "true":
            ...
            return await edit_delete(event, "**ꪎ︙  تم إيقـاف البايـو التلقائـي الآن ✓**")
        return await edit_delete(event, "**ꪎ︙  لم يتـم تفعيـل البايـو التلقائـي ✕**")
    ...
    if input_str not in END_CMDS:
        await edit_delete(            event,            f"ꪎ︙   {input_str} أمـر الإنهـاء غيـر صالـح، اذڪـر بوضـوح ما يجـب أن أنهـي !",            parse_mode=_format.parse_pre        )
