...
from . import BOTLOG, BOTLOG_CHATID, matrix

if Config.PLUGIN_CHANNEL:

    async def install():
        documentss = await matrix.get_messages(
        ...
        for module in range(total):
            ...
            if os.path.exists(f"matrix/plugins/{plugin_name}"):
                return
            downloaded_file_name = await matrix.download_media(
                await matrix.get_messages(Config.PLUGIN_CHANNEL, ids=plugin_to_install),
                "matrix/plugins/",
            )
