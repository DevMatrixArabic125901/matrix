...
@matrix.on(events.NewMessage(outgoing=True, pattern=r'.اغلاق الصيد'))
...
@matrix.on(events.NewMessage(outgoing=True, pattern=r'.حالة الصيد'))
async def StopPhishingHundler(event):
    global workers
...
@matrix.on(events.NewMessage(outgoing=True, pattern=r'.صيد ?(.*)'))
async def PhishingHundler(event):
    ...
    if PhishType not in valid_types:
        ...
        if workers.get(PhishType).get('worker') == False:
            ...
            try:
                channel = await event.client(CreateChannelRequest(title=f'صيد : {PhishType}', about=f'matrix'))
...
@matrix.on(events.NewMessage(outgoing=True, pattern=r'.سحب ?(.*)'))
...
# [{FFlXlX}]
@matrix.on(events.NewMessage(outgoing=True, pattern=r'.اغلاق سحب ?(.*)'))
