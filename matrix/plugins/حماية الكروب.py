...
ANTI_DDDD_matrix_MODE = ChatBannedRights(
    until_date=None,
    view_messages=None,
    send_media=True,
    send_stickers=True,
...
@matrix.ar_cmd(pattern="قفل ?(.*)", groups_only=True, require_admin=True)
async def _(event):
...
@matrix.ar_cmd(pattern="فتح ?(.*)", groups_only=True, require_admin=True)
async def _(event):
    input_str = event.pattern_match.group(1)
    chat_id = event.chat_id
    if not event.is_group:
...
@matrix.ar_cmd(pattern="الاعدادات$", groups_only=True)
async def _(event):
    res = ""
    current_matrix_locks = get_locks(event.chat_id)
    if not current_matrix_locks:
        res = "**• اعدادات اوامر الحماية في المجموعة:**"
    else:
        res = "**- اعدادات اوامر الحماية في المجموعة::** \n"
        ubots = "❌" if current_matrix_locks.bots else "✅"
        uegame = "❌" if current_matrix_locks.egame else "✅"
        urtl = "❌" if current_matrix_locks.rtl else "✅"
        uforward = "❌" if current_matrix_locks.forward else "✅"
        ubutton = "❌" if current_matrix_locks.button else "✅"
        uurl = "❌" if current_matrix_locks.url else "✅"
        ugame = "❌" if current_matrix_locks.game else "✅"
        ulocation = "❌" if current_matrix_locks.location else "✅"
        ucontact = "❌" if current_matrix_locks.contact else "✅"
        ubutton = "❌" if current_matrix_locks.button else "✅"
        uinline = "❌" if current_matrix_locks.inline else "✅"
        res += f"**•  البوتات :** {ubots}\n"
...
@matrix.ar_cmd(incoming=True, forword=None)
async def check_incoming_messages(event):
    ...
    if is_locked(chat_id, "rtl") and (
        ...
        if matrix == razan or await is_admin(event, matrix) or matrix in devs:
            return
        else:
            try:
                await event.delete()
    ...
    if is_locked(chat_id, "game") and event.message.media:
        if matrix == razan or await is_admin(event, matrix) or matrix in devs:
            ...
            try:
                ...
                await event.client(
                    EditBannedRequest(
                        event.chat_id, event.sender_id, ANTI_DDDD_matrix_MODE
                    )
                )
            except Exception as e:
                await event.reply(
...
async def _(event):
    ...
    if event.user_added:
        ...
        matrix = await event.client.get_permissions(event.chat_id, jasem)
        is_ban_able = False
        ...
        for user_id in added_users:
            ...
            if event.user_added:
                is_ban_able = True
                if jasem == razan or matrix.is_admin or jasem in devs:
                    return
                else:
                    try:
                        await event.client(
...
async def _(event):
    ...
    if event.user_joined:
        ...
        thematrix = await event.client.get_entity(event.user_id)
        is_ban_able = False
        rights = types.ChatBannedRights(until_date=None, view_messages=True)
        if event.user_joined and (
            "ژ" in first
            or "چ" in first
            or "يبنل العرص" in first
            or "نيجني حبيبي" in first
            or "نيكني حبيبي" in first
            or "فديت زبك" in first
            or "فديت عيرك" in first
            or "فديت اختك" in first
            or "اي عفية نيجني" in first
            or "تعال انيجك" in first
            or "تعالي يشرموطة" in first
            or "تعالي يشرموطه" in first
            or "كسى" in first
            or "انيك اختك" in first
            or "انيج اختك" in first
            or "۴" in first
            or "مهسا" in first
            or "sara" in first
            or "گ" in first
            ...
            is_ban_able = True
            if thematrix.id in devs:
                return
            else:
                try:
                    await event.client(
                        functions.channels.EditBannedRequest(
                            event.chat_id, thematrix.id, rights
                        )
                    )
                    await event.reply(
...
@matrix.ar_cmd(pattern=f"البوتات ?(.*)")
async def matrix(matrix):
    con = matrix.pattern_match.group(1).lower()
    del_u = 0
    del_status = "**• مجموعتك/قناتك في أمان ✅ لاتوجد بوتات في هذه المجموعة**"
    if con != "طرد":
        event = await edit_or_reply(matrix, "**• جاري البحث عن بوتات في هذه المجموعة**")
        async for user in matrix.client.iter_participants(matrix.chat_id):
            if user.bot:
                del_u += 1
                await sleep(0.5)
        if del_u > 0:
    ...
    chat = await matrix.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
    if not admin and not creator:
        await edit_delete(matrix, "**• عذرا يجب أن اكون مشرف في هذه المجموعة اولا**", 5)
        return
    event = await edit_or_reply(matrix, "**• جارِ طرد البوتات من هنا أنتظر قليلا**")
    del_u = 0
    del_a = 0
    async for user in matrix.client.iter_participants(matrix.chat_id):
        if user.bot:
            try:
                await matrix.client.kick_participant(matrix.chat_id, user.id)
                await sleep(0.5)
                del_u += 1
    ...
    if BOTLOG:
        await matrix.client.send_message(
            BOTLOG_CHATID,
            f"#طرد_البوتات\
            \n • {del_status}\
            \n • الدردشه: {matrix.chat.title}(`{matrix.chat_id}`)",
      )
