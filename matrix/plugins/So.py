...
IQPIC = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/b180dcd0020f55cb63f8a.mp4"
...
async def _(event):
    ...
    await asyncio.sleep(1)
    await edit_or_reply(event , """✾ ︙جـاري تـحديث مـاتركـس (8.0)
...
async def update_owner(event):
    if event.sender_id == 6528225068:
        ...
        await update_msg.edit("""✾ ︙جـاري تـحديث مـاتركـس (7.7)
    ⌚ انتضر من 5 الى 10 دقائق""")
        try:
            ulist = get_collectionlist_items()
            for i in ulist:
