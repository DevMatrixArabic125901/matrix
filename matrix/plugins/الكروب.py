...
@matrix.ar_cmd(pattern="المشرفين(?: |$)(.*)")
async def _(event):
...
async def get_users(show):
    ...
    matrixevent = await edit_or_reply(show, "**✾ جاري سحب قائمة معرّفات الأعضاء 🝛**")
    try:
        ...
        mentions += " " + str(e) + "\n"
    await edit_or_reply(matrixevent, mentions)


@matrix.ar_cmd(pattern="معلومات(?: |$)(.*)")
async def info(event):
    matrixevent = await edit_or_reply(event, "**⪼ يتمّ جلب معلومات الدردشة، إنتظر ⅏**")
    chat = await get_chatinfo(event, matrixevent)
    caption = await fetch_info(chat, event)
    try:
        await matrixevent.edit(caption, parse_mode="html")
    except Exception as e:
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, f"**✾ هناك خطأ في معلومات الدردشة ✕ : **\n`{str(e)}`"
            )
        await matrixevent.edit("**✾ حدث خطأ ما، يرجى التحقق من الأمر ⎌**")


async def get_chatinfo(event, matrixevent):
    ...
    try:
        ...
        try:
            ...
            await matrixevent.edit("**✾ لم يتمّ العثور على القناة/المجموعة ✕**")
            return None
        except ChannelPrivateError:
            await matrixevent.edit("**✾ هذه مجموعة أو قناة خاصة أو لقد تمّ حظري منه ⛞**")
            return None
        except ChannelPublicGroupNaError:
            await matrixevent.edit("**✾ القناة أو المجموعة الخارقة غير موجودة ✕**")
            return None
        except (TypeError, ValueError) as err:
            await matrixevent.edit(str(err))
            return None
    return chat_info
