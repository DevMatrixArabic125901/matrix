...
@matrix.ar_cmd(pattern="تحميل صوتي(?:\s|$)([\s\S]*)")
async def download_audio(event):
    ...
    for url in urls:
        try:
            ...
            vid_data = {"title": url, "uploader": "matrix", "formats": []}
        startTime = time()
        ...
        for _path in glob.glob(os.path.join(Config.TEMP_DIR, str(startTime), "*")):
            if _path.lower().endswith((".jpg", ".png", ".webp")):
                ...
                _fpath = _path
        if not _fpath:
            return await edit_delete(matrixevent, "**❃ غير قادر على الرفع**")
...
@matrix.ar_cmd(pattern="تحميل فيديو(?:\s|$)([\s\S]*)")
async def download_video(event):
    ...
    reply_to_id = await reply_id(event)
    for url in urls:
        ytdl_data = await ytdl_down(matrixevent, video_opts, url)
        if ytdl_down is None:
            return
        try:
            f = pathlib.Path("matrix_ytv.mp4")
            print(f)
            matrixthumb = pathlib.Path("matrix_ytv.jpg")
            if not os.path.exists(matrixthumb):
                matrixthumb = pathlib.Path("matrix_ytv.webp")
            if not os.path.exists(matrixthumb):
                matrixthumb = None
            await matrixevent.edit(
                f"**⌔∮ جار رفع الفيديو :**\
            ...
            os.remove(f)
            if matrixthumb:
                os.remove(matrixthumb)
        except TypeError:
            await asyncio.sleep(2)
    await event.delete()


@matrix.ar_cmd(pattern="انستا(?: |$)([\s\S]*)")
async def insta_dl(event):
    ...
    v2 = "@videomaniacbot"
    media_list = []
    matrixevent = await edit_or_reply(event, "**⪼ جار التحميل انتظر قليلا**")
    async with event.client.conversation(v1) as conv:
        try:
            v1_flag = await conv.send_message("/start")
        except YouBlockedUserError:
            await matrix(unblock("IgGramBot"))
            v1_flag = await conv.send_message("/start")
        ...
        await edit_or_reply(matrixevent, "**⪼ جار التحميل من مصدر اخر**")
        async with event.client.conversation(v2) as conv:
            try:
                v2_flag = await conv.send_message("/start")
            except YouBlockedUserError:
                await matrix(unblock("videomaniacbot"))
                v2_flag = await conv.send_message("/start")
            await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
...
@matrix.ar_cmd(pattern="نتائج(?: |$)(\d*)? ?([\s\S]*)")
