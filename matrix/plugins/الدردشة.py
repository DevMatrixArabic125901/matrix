...
@matrix.ar_cmd(pattern="بوتي$")
async def _(event):
    TG_BOT_USERNAME = Config.TG_BOT_USERNAME
    await event.reply(f"**❃ البوت الخاص بك هو** \n {TG_BOT_USERNAME}")


@matrix.ar_cmd(pattern="حالتي$")
async def _(event):
    text = "/start"
...
async def _(event):
    ...
    matrixevent = await edit_or_reply(event, "**بوياي جار**")
...
@matrix.ar_cmd(pattern="تفليش بالبوت$", groups_only=True)
async def banavot(event):
    ...
    try:
        await matrix(GetParticipantRequest(event.chat_id, event.sender_id))
    except UserNotParticipantError:
        pass
    spam_chats.append(chat_id)
    async for usr in matrix.iter_participants(chat_id):
        ...
        await matrix.send_message(chat_id, usrtxt)
...
@matrix.ar_cmd(pattern="الغاء التفليش", groups_only=True)
...
async def _(event):
    matrixevent = await edit_or_reply(event, "**- جار الغاء حظر جميع المستخدمين**")
    succ = 0
    total = 0
    flag = False
...
async def rm_deletedacc(show):
    ...
    if con != "تنظيف":
        ...
        if flag != " -r":
            ...
            matrixadmin = await is_admin(show.client, show.chat_id, show.client.uid)
            if not matrixadmin:
