...
from . import edit_delete, matrix, logging
...
async def autoname_loop():
    while AUTONAMESTART := gvarstatus("autoname") == "true":
        MA = time.strftime("%I:%M")
        for normal in MA:
            if normal in normzltext:
                namefont = namerzfont[normzltext.index(normal)]
                MA = MA.replace(normal, namefont)
        name = f"{MATRIX} {MA}"
        LOGS.info(name)
        try:
...
@matrix.ar_cmd(pattern="الصورة الوقتية$")
async def _(event):
    downloader = SmartDL(digitalpfp, digitalpic_path, progress_bar=False)
    downloader.start(blocking=False)
    while not downloader.isFinished():
...
@matrix.ar_cmd(pattern="اسم وقتي$")
async def _(event):
...
@matrix.ar_cmd(pattern="بايو وقتي$")
async def _(event):
...
@matrix.ar_cmd(pattern="انهاء ([\s\S]*)")
async def _(event):
