...
SONG_SEARCH_STRING = "ꪎ︙جاري البحث عن الاغنية إنتظر رجاءًا  🎧"
SONG_NOT_FOUND = "ꪎ︙لم أستطع إيجاد هذه الأغنية  ⚠️"
SONG_SENDING_STRING = "ꪎ︙قم بإلغاء حظر البوت  🚫"
...
async def ytdl_down(event, opts, url):
    try:
        await event.edit("**ꪎ︙يتم جلب البيانات إنتظر قليلا ⏳**")
        ...
        await event.edit("**ꪎ︙عُذرا هذا المحتوى قصير جدًا لتنزيله ⚠️**")
        return None
    except GeoRestrictedError:
        await event.edit(
            "**ꪎ︙الفيديو غير متاح من موقعك الجغرافي بسبب القيود الجغرافية التي يفرضها موقع الويب 🌍**"
        )
        return None
    except MaxDownloadsReached:
        await event.edit("**ꪎ︙ تم الوصول إلى الحد الأقصى لعدد التنزيلات 🛑**")
        return None
    except PostProcessingError:
        await event.edit("**ꪎ︙ كان هناك خطأ أثناء المعالجة ⚠️**")
        return None
    except UnavailableVideoError:
        await event.edit("**ꪎ︙ الوسائط غير متوفرة بالتنسيق المطلوب ⚠️**")
        return None
    except XAttrMetadataError as XAME:
        await event.edit(f"ꪎ︙ `{XAME.code}: {XAME.msg}\n{XAME.reason}`")
        return None
    except ExtractorError:
        await event.edit("**ꪎ︙ حدث خطأ أثناء استخراج المعلومات يرجى وضعها بشكل صحيح ❗️**")
        return None
    except Exception as e:
        await event.edit(f"**ꪎ︙حـدث خطأ  ⚠️ : **\n__{str(e)}__")
        return None
    return ytdl_data
...
async def collage(event):
    ...
    event = await edit_or_reply(event, "**ꪎ︙جاري الالتقاط قـد يستغـرق هـذا الأمـر عـدة دقائـق انتضر ...**")
    if not (reply and (reply.media)):
        await event.edit("**ꪎ︙تنسيـق الوسائـط غيـر مدعـوم ⚠️**")
        return
    if not os.path.isdir("./temp/"):
        os.mkdir("./temp/")
    ...
    if catinput:
        if not catinput.isdigit():
            os.remove(catsticker)
            await event.edit("**ꪎ︙إدخـالك غيـر صالـح، يرجـىٰ التحـقق مـن المساعـدة ⚠️**")
            return
        catinput = int(catinput)
        if not 0 < catinput < 10:
            os.remove(catsticker)
            await event.edit("**ꪎ︙يرجـىٰ وضـع عـدد الصـور بجانـب الأمـر إختـر رقـماً بيـن 1 إلـى 9 ✦**")
            return
    else:
        catinput = 3
    ...
    if not os.path.exists(endfile):
        ...
        return await edit_delete(
            event, f"**ꪎ︙تنسيـق الوسائـط غيـر مدعـوم، حـاول إستخـدام عـدد أصغـر  ⚠️**", 5 )
...
async def app_search(event):
    ...
    try:
        ...
        app_details += (
            "\n\n<code>ꪎ︙المطـور :</code> <a href='"
            + app_dev_link
            + "'>"
            + app_dev
            + "</a>"
        )
        app_details += "\n<code>ꪎ︙التقييـم :</code> " + app_rating.replace(
            "Rated ", "⭐ "
        ...
        app_details += (
            "\n<code>ꪎ︙المميـزات :</code> <a href='"
            + app_link
            + "'>ꪎ︙مشاهدتـه في سـوق بلـي 🝧</a>"
        )
        app_details += f"\n\n===> {ALIVE_NAME} <==="
        await event.edit(app_details, link_preview=True, parse_mode="HTML")
    except IndexError:
        await event.edit("**ꪎ︙لم يتـم العثـور على نتيجـة، الرجـاء إدخـال إسـم تطبيـق صالـح ⚠️**")
...
async def time_func(tdata):
    ...
    if len(con) > 4:
        ...
        return await edit_or_reply(tdata, f"**ꪎ︙ ألوقـت 🕛 : **{dt.now().strftime(t_form)} \n** لـتاريـخ :**{dt.now().strftime(d_form)}")
    if not timezones:
        return await edit_or_reply(tdata, "**ꪎ︙ البـلد غيـر مـوجود 𖠕**")
    if len(timezones) == 1:
        ...
        if tz_num:
            ...
            return_str = f"**ꪎ︙ `{c_name}` لها مناطق زمنية متعددة :**\n\n"

            for i, item in enumerate(timezones):
                return_str += f"`{i+1}. {item}`\n"

            return_str += "\n**ꪎ︙ اختر واحدة عن طريق كتابة الرقم : **"
            return_str += "**ꪎ︙ في الأمر .**\n"
            return_str += f"**ꪎ︙ الأمر هوه: .وقت** {c_name} 2`"
...
async def corona(event):
    ...
    catevent = await edit_or_reply(event, "**ꪎ︙يتـم جلـب معلومـات فـايروس كـورونا فـي البلـد المحـدد 🔎**")
    ...
    if country_data:
        ...
        data = ""
        data += f"\nꪎ︙ الاصابات المؤكده 😟 : <code>{hmm1}</code>"
        data += f"\nꪎ︙ الاصابات المشبوهه 🥺 : <code>{country_data['active']}</code>"
        data += f"\nꪎ︙ الوفيات ⚰️ : <code>{hmm2}</code>"
        data += f"\nꪎ︙ الحرجه 😔 : <code>{country_data['critical']}</code>"
        data += f"\nꪎ︙ حالات الشفاء 😊 : <code>{country_data['recovered']}</code>"
        data += f"\nꪎ︙ اجمالي الاختبارات 📊 : <code>{country_data['total_tests']}</code>"
        data += f"\nꪎ︙ الاصابات الجديده 🥺 : <code>{country_data['new_cases']}</code>"
        data += f"\nꪎ︙ الوفيات الجديده ⚰️ : <code>{country_data['new_deaths']}</code>"
        await catevent.edit(
            "<b>ꪎ︙ معلومـات فـايروس كـورونا. 💉 لـ {}:{}</b>".format(country, data),
            parse_mode="html",
        )
    else:
        ...
        if data:
            ...
            result = f"<b>Corona virus info of {data['state_name']}\
                \nꪎ︙ الاصابات المؤكده 😟 : <code>{data['new_positive']}</code>\
                \nꪎ︙ الاصابات المشبوهه 🥺 : <code>{data['new_active']}</code>\
                \nꪎ︙ الوفيات ⚰️ : <code>{data['new_death']}</code>\
                \nꪎ︙ حالات الشفاء 😊 : <code>{data['new_cured']}</code>\
                \nꪎ︙ اجمالي الاختبارات 📊  : <code>{cat1}</code>\
                \nꪎ︙ الاصابات الجديده 🥺 : <code>{cat2}</code>\
                \nꪎ︙ الوفيات الجديده ⚰️ : <code>{cat3}</code> </b>"
...
async def shazamcmd(event):
    ...
    if not reply or not mediatype or mediatype not in ["Voice", "Audio"]:
        return await edit_delete(
            event, "**ꪎ︙قم بالرد على الرسالة الصوتية لعكس البحث عن هذه الأغنية  ♻️**"
        )
    catevent = await edit_or_reply(event, "**ꪎ︙جاري بحث معلومات المقطع الصوتي  📲**")
    try:
        ...
        LOGS.error(e)
        return await edit_delete(
            catevent, f"**ꪎ︙هناك خطأ عند محاولة عكس الأغنية  ⚠️ :**\n__{str(e)}__"
        )
...
async def gsearch(q_event):
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                return await edit_delete(catevent, f"**ꪎ︙خطـأ ⚠️ :**\n`{str(e)}`", time=10)
    msg = ""
    for i in range(lim):
    ...
    await edit_or_reply(
        ...
        link_preview=False,
        aslink=True,
        linktext=f"**ꪎ︙نتائـج البحـث عـن الإستعـلام ⎙ ** `{match}` :",
    )
    if BOTLOG:
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        catevent = await edit_or_reply(event, "**ꪎ︙وسائـط ما قبـل المعالجـة ␥**")
        ...
        if previous_message.media:
            ...
            the_location = google_rs_response.headers.get("Location")
        await catevent.edit("**ꪎ︙تم العثـور على نتيجـة بحـث جـوجـل ✓**")
        ...
        try:
            ...
            return await edit_delete(
                catevent, "**ꪎ︙غيـر قـادر على إيجـاد صـور مشابـهه !**"
            )
        end = datetime.now()
        ms = (end - start).seconds
        OUTPUT_STR = """{img_size}
<b>ꪎ︙بحـث ممڪـن ذو صلـة 🜉  : </b> <a href="{prs_url}">{prs_text}</a> 
<b>ꪎ︙مزيـد من المعلومـات 🝰 : </b> إفتـح هـذا ␥ <a href="{the_location}">Link</a> 
<i>ꪎ︙تم الجلـب في {ms} ثانيـة ⏱</i>""".format(
            **locals()
        )
    else:
        catevent = event
...
async def google_search(event):
    ...
    if not input_str:
        return await edit_delete(
            event, "**ꪎ︙ما الذي يجـب أن أبحـث عنـه؟ يرجـىٰ إعطـاء معلومـات عن البحـث ⚠️**"
        )
    input_str = deEmojify(input_str).strip()
    if len(input_str) > 195 or len(input_str) < 1:
        return await edit_delete(
            event,
            "**ꪎ︙لقـد تجـاوز إستعـلام البحـث 200 حـرف أو أن إستعـلام البحـث فـارغ ⚠️**",
        )
    query = "#12" + input_str
...
async def _(event):
    ...
    if not reply:
        return await edit_delete(
            event, "**ꪎ︙ يجـب عليـك الرد عـلى الملصق لتحويـله الـى صورة ⚠️**"
        )
    output = await _cattools.media_to_pic(event, reply)
    if output[1] is None:
        return await edit_delete(
            output[0], "**ꪎ︙ غـير قـادر على تحويل الملصق إلى صورة من هـذا الـرد ⚠️**"
        )
...
async def _(event):
    ...
    if not reply:
        return await edit_delete(
            event, "**ꪎ︙ يجـب عليـك الرد عـلى الصـورة لتحويـلها الـى مـلصق ⚠️**"
        )
    output = await _cattools.media_to_pic(event, reply)
    if output[1] is None:
        return await edit_delete(
            output[0], "**ꪎ︙ غـير قـادر على استـخراج الـملصق من هـذا الـرد ⚠️**"
        )
...
async def _(event):
    if not event.reply_to_msg_id:
        await edit_or_reply(event, "**ꪎ︙ يـجب الـرد على اي مـلف اولا ⚠️**")
        return
    ...
    if not reply_message.media:
        ...
        return
    input_str = event.pattern_match.group(1)
    event = await edit_or_reply(event, "**ꪎ︙ يتـم التـحويل انتـظر قليـلا ⏱**")
    try:
        start = datetime.now()
        c_time = time.time()
        downloaded_file_name = await event.client.download_media(
        ...
        ms = (end - start).seconds
        await event.edit(
            "**ꪎ︙ التحـميل الى `{}`  في {} من الثواني ⏱**".format(downloaded_file_name, ms)
        )
...
async def _(event):
    ...
    if mediatype and mediatype != "video":
        return await edit_delete(event, "**ꪎ︙ يجـب عليك الـرد على فيديو اولا لتحـويله ⚠️**")
    ...
    if not args:
        ...
        try:
            ...
            args = 2.0
    catevent = await edit_or_reply(event, "**ꪎ︙ يتـم التحويل الى متـحركه انتـظر ⏱**")
    ...
    if result is None:
        return await edit_delete(event, "**ꪎ︙ عـذرا لا يمكـنني تحويل هذا الى متـحركة ⚠️**")
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    reply = await event.get_reply_message()
    if not reply:
        return await edit_delete(event, "**ꪎ︙قـم بالـرد على وسائـط مدعومـة !**")
    media_type(reply)
    catevent = await edit_or_reply(event, "**ꪎ︙جـاري تحويل الملصق الى فيديو مرئي دائـري ⌯**")
    output = await _cattools.media_to_pic(event, reply, noedits=True)
    if output[1] is None:
        return await edit_delete(
            output[0], "**ꪎ︙تعـذّر إستخـراج الصـورة من الرسالـة التي تـم الـرّد عليهـا ✕**"
        )
    meme_file = convert_toimage(output[1])
    ...
    try:
        ...
        return await edit_delete(output[0], f"**ꪎ︙خطـأ ⚠️ :**\n__{str(e)}__")
    output = io.BytesIO()
    ...
    if output is None:
        return await edit_delete(catevent, "**ꪎ︙تعـذّر صنـع صـورة متحرڪـة دوارة ✕**")
...
async def video_catfile(event):  # sourcery no-metrics
    ...
    if mediatype == "Round Video":
        return await edit_delete(
            event,
            "ꪎ︙الوسائـط التي تم الـرد عليهـا هـي بالفعـل في شڪـل دائـري، أعـد التحـقق !",
        )
    if mediatype not in ["Photo", "Audio", "Voice", "Gif", "Sticker", "Video"]:
        return await edit_delete(event, "**ꪎ︙لم يتـم العثـور على وسائـط مدعومـة !**")
    flag = True
    catevent = await edit_or_reply(event, "**ꪎ︙جـاري التحويـل إلى شڪـل دائـري ⌯**")
    ...
    if (
        ...
        if os.path.exists(PATH):
            ...
            uploaded = await event.client.fast_upload_file(
                file=ul,
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, catevent, c_time, "**ꪎ︙قـم بالـرد على وسائـط مدعومـة !**")
                ),
            )
            ul.close()
            media = types.InputMediaUploadedDocument(
...
async def get(event):
    ...
    try:
        ...
        return await output[0].edit(f"**ꪎ︙ عذرا هناك خطأ : **\n`{str(exc)}`")
...
async def get(event):
    ...
    if mediatype != "Document":
        return await edit_delete(
            event, "**ꪎ︙يبـدو أن هـذا الملـف غـير قابـل للڪتابـة،  يرجـى الـرد على ملـف قابـل للكتابـة !**"
        )
    ...
    if file_content == "":
        try:
            ...
            if os.path.exists(file_loc):
                os.remove(file_loc)
            return await edit_delete(event, f"**ꪎ︙خطـأ ⚠️**\n__{str(e)}__")
    await edit_or_reply(
        event,
        file_content,
        ...
        aslink=True,
        noformat=True,
        linktext="**ꪎ︙يسمـح تليڪرام فقـط بـ 4096 حرفًـا في الرسالـة الواحـدة، ولڪن الملـف الـذي قمـت بالـرد عليـه يحتـوي على أڪثـر مـن ذلـك بڪثيـر، لذلـك (( لصقها على رابط لصق )) غيرها انت)) !**",
    )
    if os.path.exists(file_loc):
        os.remove(file_loc)
...
async def on_file_to_photo(event):
    ...
    try:
        ...
        return await edit_delete(event, "**ꪎ︙هـذه ليسـت صـورة !**")
    if not image.mime_type.startswith("image/"):
        return await edit_delete(event, "**ꪎ︙هـذه ليسـت صـورة !**")
    if image.mime_type == "image/webp":
        return await edit_delete(event, "**ꪎ︙لتحويـل الملصـق إلى صـورة إستخـدم الأمـر  ⩥ :**  `.تحويل ملف صوره`")
    if image.size > 10 * 1024 * 1024:
        return  # We'd get PhotoSaveFileInvalidError otherwise
    catt = await edit_or_reply(event, "**ꪎ︙جـاري التحويـل  ↯**")
...
async def _(event):  # sourcery no-metrics
    ...
    if not input_str:
        ...
        if len(loc) == 2:
            if 0 < loc[0] < 721:
                ...
                return await edit_delete(event, "**ꪎ︙إستخـدم جـودة النطـاق مـن 0 إلى 721 ✦**")
            if 0 < loc[1] < 20:
                quality = loc[1].strip()
            else:
                return await edit_delete(event, "**ꪎ︙إستخـدم جـودة النطـاق مـن 0 إلى 20 ✦**")
        if len(loc) == 1:
            if 0 < loc[0] < 721:
                quality = loc[0].strip()
            else:
    ...
    if not catreply or not catreply.media or not catreply.media.document:
        return await edit_or_reply(event, "**ꪎ︙هـذا ليـس ملصـق متحرك   !**")
    if catreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "**ꪎ︙هـذا ليـس ملصـق متحرك  !**")
    catevent = await edit_or_reply(
        event,
        "ꪎ︙جـاري تحويـل هـذا الملصـق إلى صـورة متحرڪـة، قـد يستغـرق هـذا بضـع دقائـق ✦",
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    if not reply or not mediatype or mediatype not in ["Photo", "Sticker"]:
        return await edit_delete(event, "**ꪎ︙قم بالـرد على صـورة أو ملصـق لجعلهـا صـورة متحرڪـة **")
    if mediatype == "Sticker" and reply.document.mime_type == "application/i-tgsticker":
        return await edit_delete(
            event,
            "**ꪎ︙قم بالـرد على صـورة أو ملصـق لجعلهـا صـورة متحرڪـة، الملصقـات المتحرڪـة غيـر مدعومـة !**",
        )
    args = event.pattern_match.group(1)
    args = "i" if not args else args.replace("-", "")
    catevent = await edit_or_reply(event, "**ꪎ︙جـاري صنـع صـورة متحرڪـة من الوسائـط التي قمـت بالـرد عليهـا ↯**")
    imag = await _cattools.media_to_pic(event, reply, noedits=True)
    if imag[1] is None:
        return await edit_delete(
            imag[0], "**ꪎ︙تعـذّر إستخـراج الصـورة من الرسالـة التي تـم الـرّد عليهـا ✕**"
        )
    ...
    if output is None:
        await edit_delete(
            catevent, "**ꪎ︙حـدث خطـأ مـا في الوسائـط، لا أستطيـع تحويلهـا إلى صـورة متحرڪـة !**"
        )
...
async def _(event):
    catevent = await edit_or_reply(event, "**ꪎ︙جـاري المعالجـة ⌯**")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"**ꪎ︙تـمّ إنشـاء تليجـراف جديـد ✓ :** {auth_url} \n **للجلسـة الحاليـة، لا تقـم بإعطـاء هـذا الرابـط إلى أي أحـد، حتى وإن قـال بأنّـه موظـف لـدى تليڪـرام !**",
        )
    optional_title = event.pattern_match.group(5)
    if not event.reply_to_msg_id:
        return await catevent.edit(
            "**ꪎ︙قـم بالـردّ على رسالـة للحصـول على رابـط صـورة تليجـراف دائـم ☍**",
        )
    ...
    if input_str in ["ميديا", "م"]:
        ...
        await catevent.edit(f"**ꪎ︙تـم التحميـل إلى**  {downloaded_file_name}`")
        if downloaded_file_name.endswith((".webp")):
            resize_image(downloaded_file_name)
        try:
            media_urls = upload_file(downloaded_file_name)
        except exceptions.TelegraphException as exc:
            await catevent.edit(f"**ꪎ︙حـدث خـطأ مـا ✕ : **\n`{str(exc)}`")
            os.remove(downloaded_file_name)
        else:
            end = datetime.now()
            ms = (end - start).seconds
            os.remove(downloaded_file_name)
            await catevent.edit(
                 f"**ꪎ︙ الرابـط ☍ : ** [Press here](https://telegra.ph{media_urls[0]})\
                    \n**ꪎ︙الوقـت المستغـرق ⏱  : ** `{ms} الثوانـي.`",
                link_preview=True,
            )
        ...
        await catevent.edit(f"**ꪎ︙ الرابـط ☍ : ** [Press here]({cat})\n**ꪎ︙الوقـت المستغـرق ⏱  : ** `{ms} الثوانـي.`", link_preview=True)
@iqthon.on(admin_cmd(pattern="تحويل فديو متحركه ?([0-9.]+)?$"))
async def _(event):
    reply = await event.get_reply_message()
    mediatype = media_type(event)
    if mediatype and mediatype != "video":
        return await edit_delete(event, "**ꪎ︙حـدث خطـأ مـا في الوسائـط، لا أستطيـع تحويلهـا إلى صـورة متحرڪـة !**")
    args = event.pattern_match.group(1)
    if not args:
        args = 2.0
    else:
        try:
            ...
            args = 2.0
    catevent = await edit_or_reply(event, "**ꪎ︙جـاري التحويـل إلى صـورة متحرڪة انتضر دقائق  ↯**")
    inputfile = await reply.download_media()
    outputfile = os.path.join(Config.TEMP_DIR, "vidtogif.gif")
    result = await vid_to_gif(inputfile, outputfile, speed=args)
    if result is None:
        return await edit_delete(event, "**ꪎ︙غيـر قـادر على تحويلهـا إلى صـورة متحرڪة !**")
