...
PP_TOO_SMOL = "**ꪎ︙ الصورة صغيرة جدًا  📸** ."
PP_ERROR = "**ꪎ︙ فشل أثناء معالجة الصورة  📵** ."
NO_ADMIN = "**ꪎ︙ أنا لست مشرف هنا ** ."
NO_PERM = "**ꪎ︙ ليس لدي أذونات كافية  🚮** ."
CHAT_PP_CHANGED = "**ꪎ︙ تغيّرت صورة الدردشة  🌅** ."
INVALID_MEDIA = "**ꪎ︙ملحق غير صالح  📳** ."
IMOGE_matrix = "ꪎ︙ "
NO_ADMIN = "**ꪎ︙عذرا لست ادمن هنا **"
NO_PERM = "**ꪎ︙ليس لدي صلاحيات ادمن كافية! **"


@matrix.on(admin_cmd(pattern=r"المحظورين?(.*)"))
...
@matrix.on(admin_cmd(pattern="تشفير الملف ?(.*)"))
async def pyZip(e):
    if e.fwd_from:
        return
    ...
    zip_ = f"{nem_}.zip" if nem_ else "matrix_Zip.zip"
    password = pass_ if pass_ else "matrix"
...
@matrix.on(admin_cmd(pattern=f"{MUTE}(?:\s|$)([\s\S]*)"))
async def mutematrix(event):
    if event.is_private:
...
@matrix.on(admin_cmd(pattern=f"{UNMUTE}(?:\s|$)([\s\S]*)"))
...
@matrix.on(admin_cmd(pattern=r"المشرفين(?: |$)(.*)"))
async def _(event):
    "لإظهـار قائمـة المشرفيـن  ✪"
    mentions = "**ꪎ︙  مشرفيـن هـذه المجموعـة  ✪**: \n"
    reply_message = await reply_id(event)
    input_str = event.pattern_match.group(1)
    to_write_chat = await event.get_input_chat()
    chat = None
    if input_str:
        mentions = f"**ꪎ︙ مشرفيـن فـي → :** {input_str} **مـن المجموعـات ⌂ :** \n"
        try:
        ...
        if not event.is_group:
            return await edit_or_reply(event, "**ꪎ︙  هـذه ليسـت مجموعـة ✕**")
    try:
...
@matrix.on(admin_cmd(pattern=r"البوتات?(.*)"))
async def _(event):
    mentions = "**ꪎ︙ البـوتات في هذه الـمجموعة 🝰 : ** \n"
    input_str = event.pattern_match.group(1)
    if not input_str:
        chat = await event.get_input_chat()
    else:
        mentions = "**ꪎ︙ البوتـات في {} من المجموعات 🝰 : ** \n".format(input_str)
        try:
            chat = await event.client.get_entity(input_str)
        except Exception as e:
...
@matrix.on(admin_cmd(pattern=r"الأعضاء(?: |$)(.*)"))
async def get_users(show):
    mentions = "**مستخدمين هذه المجموعة**: \n"
    await reply_id(show)
    input_str = show.pattern_match.group(1)
    if input_str:
        mentions = "**ꪎ︙ الأعضاء في {} من المجموعات 𖤍  :** \n".format(input_str)
        try:
        ...
        if not show.is_group:
            return await edit_or_reply(show, "**ꪎ︙ هـذه ليسـت مجموعـة ✕**")
    catevent = await edit_or_reply(show, "**ꪎ︙ جـاري سحـب قائمـة معرّفـات الأعضـاء 🝛**")
    try:
        if show.pattern_match.group(1):
            async for user in show.client.iter_participants(chat.id):
                if user.deleted:
                    mentions += f"\n**ꪎ︙ الحسـابات المحذوفـة ⌦** `{user.id}`"
                else:
...
@matrix.on(admin_cmd(pattern=r"معلومات(?: |$)(.*)"))
async def info(event):
    catevent = await edit_or_reply(event, "**ꪎ︙ يتـمّ جلـب معلومـات الدردشـة، إنتظـر ⅏**")
    ...
    try:
        ...
        if BOTLOG:
            await event.client.send_message(BOTLOG_CHATID, f"**ꪎ︙ هنـاك خطـأ في معلومـات الدردشـة ✕ : **\n`{str(e)}`")
        await catevent.edit("**ꪎ︙  حـدث خـطأ مـا، يرجـى التحقق من الأمـر ⎌**")
async def get_chatinfo(event, catevent):
    ...
    try:
        ...
        try:
            ...
            await catevent.edit("**ꪎ︙ لـم يتـمّ العثـور على القنـاة/المجموعـة ✕**")
            return None
        except ChannelPrivateError:
            await catevent.edit(
                '**ꪎ︙  هـذه مجموعـة أو قنـاة خاصـة أو لقد تمّ حظـري منه ⛞**'
            )
            return None
        except ChannelPublicGroupNaError:
            await catevent.edit("**ꪎ︙ القنـاة أو المجموعـة الخارقـة غيـر موجـودة ✕**")
            return None
...
async def get_chatinfo(event):
    ...
    try:
        ...
        try:
            ...
            await event.reply("**ꪎ︙  لم يتم العثور على المجموعة او القناة**")
            return None
        except ChannelPrivateError:
            await event.reply(
                "**ꪎ︙  لا يمكنني استخدام الامر من الكروبات او القنوات الخاصة**"
            )
            return None
        except ChannelPublicGroupNaError:
            await event.reply("**ꪎ︙  لم يتم العثور على المجموعة او القناة**")
            ...
            await event.reply("**ꪎ︙  رابط الكروب غير صحيح**")
            return None
    return chat_info
...
async def fetch_info(chat, event):  # sourcery no-metrics
    ...
    caption = "<b>ꪎ︙ معلومـات الدردشـة  🝢 :</b>\n"
    caption += f"ꪎ︙ الآيـدي  : <code>{chat_obj_info.id}</code>\n"
    if chat_title is not None:
        caption += f"ꪎ︙ إسـم المجموعـة  :{chat_title}\n"
    if former_title is not None:  # Meant is the very first title
        caption += f"ꪎ︙ الإسم السابـق  : {former_title}\n"
    if username is not None:
        caption += f"ꪎ︙ نـوع المجموعـة ⌂ : مجموعـة عامّـة  \n"
        caption += f"ꪎ︙ الرابـط  : \n {username}\n"
    else:
        caption += f"ꪎ︙  نـوع المجموعـة ⌂ : مجموعـة عامّـة  \n"
    if creator_username is not None:
        caption += f"ꪎ︙  المالـك  :  {creator_username}\n"
    elif creator_valid:
        caption += ('ꪎ︙  المالـك  : <a href="tg://user?id={creator_id}">{creator_firstname}</a>\n')
    if created is not None:
        caption += f"ꪎ︙ تاريـخ الإنشـاء  : \n <code>{created.date().strftime('%b %d, %Y')} - {created.time()}</code>\n"
    else:
        caption += f"ꪎ︙ الإنتـاج  :   <code>{chat_obj_info.date.date().strftime('%b %d, %Y')} - {chat_obj_info.date.time()}</code> {warn_emoji}\n"
    caption += f"ꪎ︙ آيـدي قاعـدة البيانـات : {dc_id}\n"
    if exp_count is not None:
        chat_level = int((1 + sqrt(1 + 7 * exp_count / 14)) / 2)
        caption += f"ꪎ︙ الأعضـاء : <code>{chat_level}</code>\n"
    if messages_viewable is not None:
        caption += f"ꪎ︙ الرسائـل التي يمڪن مشاهدتها : <code>{messages_viewable}</code>\n"
    if messages_sent:
        caption += f"ꪎ︙ الرسائـل المرسلـة  :<code>{messages_sent}</code>\n"
    elif messages_sent_alt:
        caption += f"ꪎ︙ الرسـائل المرسلة: <code>{messages_sent_alt}</code> {warn_emoji}\n"
    if members is not None:
        caption += f"ꪎ︙ الأعضـاء : <code>{members}</code>\n"
    if admins is not None:
        caption += f"ꪎ︙ المشرفيـن : <code>{admins}</code>\n"
    if bots_list:
        caption += f"ꪎ︙ البـوتات : <code>{bots}</code>\n"
    if members_online:
        caption += f"ꪎ︙ المتصليـن حـالياً : <code>{members_online}</code>\n"
    if restrcited_users is not None:
        caption += f"ꪎ︙ الأعضـاء المقيّديـن : <code>{restrcited_users}</code>\n"
    if banned_users is not None:
        caption += f"ꪎ︙ الأعضـاء المحظوريـن : <code>{banned_users}</code>"
    if group_stickers is not None:
        caption += f'{chat_type} ꪎ︙ الملصقـات : <a href="t.me/addstickers/{chat.full_chat.stickerset.short_name}">{group_stickers}</a>'
    caption += "\n"
    if not broadcast:
        caption += f"ꪎ︙ الوضـع البطيئ : {slowmode}"
        if (
            ...
            caption += "\n"
        caption += f"ꪎ︙ الـمجموعـة الخارقـة  : {supergroup}\n"
    if hasattr(chat_obj_info, "restricted"):
        caption += f"ꪎ︙ المقيّـد : {restricted}"
        if chat_obj_info.restricted:
            caption += f"> : {chat_obj_info.restriction_reason[0].platform}\n"
            caption += f"> ꪎ︙ السـبب  : {chat_obj_info.restriction_reason[0].reason}\n"
            caption += f"> ꪎ︙ النّـص  : {chat_obj_info.restriction_reason[0].text}\n\n"
        else:
            caption += "\n"
    if hasattr(chat_obj_info, "scam") and chat_obj_info.scam:
        caption += "ꪎ︙ السارقيـن : <b>Yes</b>\n"
    if hasattr(chat_obj_info, "verified"):
        caption += f"ꪎ︙ الحسابـات الموثقـة   : {verified}\n"
    if description:
        caption += f"ꪎ︙ الوصـف  : \n<code>{description}</code>\n"
    return caption

@matrix.on(admin_cmd(pattern=f"{addition} ?(.*)"))
async def iq(event):
    sender = await event.get_sender()
    me = await event.client.get_me()
    if not sender.id == me.id:
        kno = await event.reply("**ꪎ︙  تتـم العـملية انتظـࢪ قليلا ..**")
    else:
        kno = await event.edit("**ꪎ︙  تتـم العـملية انتظـࢪ قليلا ..**.")
    matrix = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await kno.edit("**ꪎ︙  لا يمكننـي اضافـة المـستخدمين هـنا**")
    s = 0
    f = 0
    error = "None"

    await kno.edit("**ꪎ︙  حـالة الأضافة:**\n\n**ꪎ︙  تتـم جـمع معـلومات الـمستخدمين 🔄 ...⏣**")
    async for user in event.client.iter_participants(matrix.full_chat.id):
        try:
            if error.startswith("Too"):
                return (
                    await kno.edit(
                        f"**ꪎ︙  حـالة الأضـافة انتـهت مـع الأخـطاء**\n- (**ربـما هـنالك ضغـط عـلى الأمر حاول مجـدا لاحقـا **) \n**ꪎ︙  الـخطأ ** : \n`{error}`\n\nꪎ︙  اضالـة `{s}` \nꪎ︙  خـطأ بأضافـة `{f}`"
                    ),
                )
            await event.client(
            ...
            s = s + 1
            await kno.edit(f"**ꪎ︙  تتـم الأضـافة :**\n\nꪎ︙  اضـيف `{s}` \nꪎ︙   خـطأ بأضافـة `{f}` \n\n**ꪎ︙  × اخـر خـطأ:** `{error}`")
        except Exception as e:
            error = str(e)
            f = f + 1
    return await kno.edit(f"**ꪎ︙  اڪتـملت الأضافـة ✅** : \n\nꪎ︙  تـم بنجـاح اضافـة `{s}` \nꪎ︙  خـطأ بأضافـة `{f}`")
    
@matrix.on(admin_cmd(pattern=f"{TFLASH}(.*)"))
async def _(event):
    result = await event.client(functions.channels.GetParticipantRequest(event.chat_id, event.client.uid))
    if not result:
        return await edit_or_reply(event, "**ꪎ︙  ليس لديك صلاحيه حظر في هذا الدردشة**")
    matrixevent = await edit_or_reply(event, "**ꪎ︙ جاري تفليش مجموعتك أنتظر قليلآ 🚮**")
    ...
    async for user in event.client.iter_participants(event.chat_id):
        ...
        try:
            ...
            LOGS.info(str(e))
    await matrixevent.edit(f"**ꪎ︙  تم بنجاح تفليش مجموعتك من {total} الاعضاء 🚮**")
    
async def ban_user(chat_id, i, rights):
    try:
        await matrix(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)


@matrix.on(admin_cmd(pattern=f"{LEFT}(.*)"))
async def kickme(leave):
    await leave.edit("**ꪎ︙  جـاري مـغادرة المجـموعة مـع السـلامة  🚶‍♂️  ..**")
    await leave.client.kick_participant(leave.chat_id, "me")

@matrix.on(admin_cmd(pattern=f"{REMOVEBAN}(.*)"))
async def _(event):
    catevent = await edit_or_reply(event, "**ꪎ︙   إلغاء حظر جميع الحسابات المحظورة في هذه المجموعة 🆘**")
    succ = 0
    total = 0
    flag = False
    chat = await event.get_chat()
    async for i in event.client.iter_participants(event.chat_id, filter=ChannelParticipantsKicked, aggressive=True):
        ...
        try:
            ...
            LOGS.warn(f"**ꪎ︙  هناك ضغط كبير بالاستخدام يرجى الانتضار .. ‼️ بسبب  : {e.seconds} **")
            await catevent.edit(f"**ꪎ︙  {readable_time(e.seconds)} مطلـوب المـعاودة مـرة اخـرى للـمسح 🔁 **")
            ...
            try:
                if succ % 10 == 0:
                    await catevent.edit(f"**ꪎ︙  جـاري مسـح المحـظورين ⭕️  : \n {succ} الحسـابات الـتي غيـر محظـورة لحـد الان.**")
            except MessageNotModifiedError:
                pass
    await catevent.edit(f"**ꪎ︙  تـم مسـح المحـظورين مـن أصـل 🆘 :**{succ}/{total} \n اسـم المجـموعـة 📄 : {chat.title}")

@matrix.on(admin_cmd(pattern=f"المحذوفين ?([\s\S]*)"))
async def rm_deletedacc(show):
    con = show.pattern_match.group(1).lower()
    del_u = 0
    del_status = "**ꪎ︙ لا توجـد حـسابات محذوفـة في هـذه المجموعـة !**"
    if con != "تنظيف":
        event = await edit_or_reply(show, "**ꪎ︙ جـاري البحـث عـن الحسابـات المحذوفـة ⌯**")
        async for user in show.client.iter_participants(show.chat_id):
            if user.deleted:
                del_u += 1
                await sleep(0.5)
        if del_u > 0:
            del_status = f"**ꪎ︙ لقد وجـدت  {del_u}  من  حسابـات محذوفـة في هـذه المجموعـة لحذفهـم إستخـدم الأمـر  ⩥ :  `.المحذوفين تنظيف`"
    ...
    creator = chat.creator
    if not admin and not creator:
        await edit_delete(show, "ꪎ︙ أنـا لسـت مشـرفـاً هنـا !", 5)
        return
    event = await edit_or_reply(show, "**ꪎ︙ جـاري حـذف الحسـابات المحذوفـة ⌯**")
    del_u = 0
    del_a = 0
    async for user in show.client.iter_participants(show.chat_id):
        if user.deleted:
            try:
                ...
                await edit_delete(event, "**ꪎ︙   ليس لدي صلاحيات الحظر هنا**", 5)
                return
            except UserAdminInvalidError:
                del_a += 1
    if del_u > 0:
        del_status = f"**ꪎ︙ تـم حـذف  {del_u}  الحسـابات المحذوفـة ✓**"
    if del_a > 0:
        del_status = f"**ꪎ︙ تـم حـذف {del_u} الحسـابات المحذوفـة، ولڪـن لـم يتـم حذف الحسـابات المحذوفـة للمشرفيـن !**"
    ...
    if BOTLOG:
        await show.client.send_message(
            ...
            f"**ꪎ︙ تنظيف :**\
            \nꪎ︙  {del_status}\
            \n*ꪎ︙ المحادثـة ⌂** {show.chat.title}(`{show.chat_id}`)",
        )

@matrix.on(admin_cmd(pattern=r"احصائيات الاعضاء ?([\s\S]*)"))
async def _(event):  # sourcery no-metrics
    input_str = event.pattern_match.group(1)
    if input_str:
        chat = await event.get_chat()
        if not chat.admin_rights and not chat.creator:
            await edit_or_reply(event, "**ꪎ︙  انت لست مشرف هنا**")
            return False
    p = 0
    b = 0
    c = 0
    ...
    q = 0
    r = 0
    et = await edit_or_reply(event, "**ꪎ︙  جـاري البحـث عـن قوائـم المشارڪيـن ⌯**")
    async for i in event.client.iter_participants(event.chat_id):
        ...
        if isinstance(i.status, UserStatusEmpty):
            ...
            if "y" in input_str:
                ...
                if status:
                    ...
                    await et.edit("**ꪎ︙ أحتـاج إلى صلاحيـات المشـرف لإجـراء هـذا الأمـر !**")
                    e.append(str(e))
                    break
        ...
        if i.bot:
            ...
            if "b" in input_str:
                status, e = await ban_user(event.chat_id, i, rights)
                if not status:
                    await et.edit("**ꪎ︙  احتاج الى صلاحيات المشرفين للقيام بهذا الامر **")
                    e.append(str(e))
                    break
                else:
                    c += 1
            ...
            n += 1
    if input_str:
        required_string = """**ꪎ︙  الـمطرودين {} / {} الأعـضاء
ꪎ︙  الحـسابـات المـحذوفة: {}
ꪎ︙  حـالة المستـخدم الفـارغه: {}
ꪎ︙  اخر ظهور منذ شـهر: {}
ꪎ︙  اخر ظـهور منـذ اسبوع: {}
ꪎ︙  غير متصل: {}
ꪎ︙  المستخدمين النشطون: {}
ꪎ︙  اخر ظهور قبل قليل: {}
ꪎ︙  البوتات: {}
ꪎ︙  مـلاحظة: {}**"""
...
ꪎ︙  اخر ظهور قبل قليل: {}
ꪎ︙  البوتات: {}
ꪎ︙  مـلاحظة: {} **""".format(            p, d, y, m, w, o, q, r, b, n        )    )    
...
@matrix.on(admin_cmd(pattern="تقيد(?:\s|$)([\s\S]*)",))
...
@matrix.iq_cmd(pattern="اكتم(?:\s|$)([\s\S]*)",)
async def tmuter(event):  # sourcery no-metrics
    catevent = await edit_or_reply(event, "**ꪎ︙جاري كتمه مؤقتا ....**")
    user, reason = await get_user_from_event(event, catevent)
    if not user:
        return
    if not reason:
        return await catevent.edit("**ꪎ︙رجاء طريقه كتابه الامر خاطئه قم بروئيه قناه شروحات الاوامر : @l3ll3**")
    ...
    try:
        ...
        return await catevent.edit("ꪎ︙ أنك لست مسؤولاً  فتأكد ان لديك صلاحيه حذف الرسائل")
    except UserAdminInvalidError:
        return await catevent.edit(
            "ꪎ︙ أنك لست مسؤولاً  فتأكد ان لديك صلاحيه حذف الرسائل"
        )
    except Exception as e:
        return await catevent.edit(f"`{e}`")


@matrix.on(admin_cmd(pattern=r"احظر(?:\s|$)([\s\S]*)",))
async def tban(event):  # sourcery no-metrics
    catevent = await edit_or_reply(event, "**ꪎ︙جاري حظر الشخص مؤقتا .... **")
    user, reason = await get_user_from_event(event, catevent)
    if not user:
        return
    if not reason:
    ...
    if not ctime:
        return
    if user.id == event.client.uid:
        return await catevent.edit("ꪎ︙عذرا لايمكنني حظر نفسي")
    await catevent.edit("!")
    try:
        await event.client(
            EditBannedRequest(
        ...
        return await catevent.edit(
            "ꪎ︙إما أنك لست مسؤولاً أو أنك حاولت حظر مسؤول لم تقم بترقيته فتأكد ان لديك صلاحيه الحظر"
        )
...
@matrix.on(admin_cmd(pattern=r"حظر عام(?:\s|$)([\s\S]*)",))
async def catgban(event):  # sourcery no-metrics
    cate = await edit_or_reply(event, "**ꪎ︙جاري حظر الشخص عام .... **")
    start = datetime.now()
    ...
    sandy = len(san)
    if sandy == 0:
        return await edit_delete(cate, "ꪎ︙إما أنك لست مسؤولاً أو أنك حاولت حظر مسؤول لم تقم بترقيته فتأكد ان لديك صلاحيه الحظر")
    await cate.edit(
...
@matrix.on(admin_cmd(pattern=r"الغاء حظر العام(?:\s|$)([\s\S]*)",))
async def catgban(event):
    cate = await edit_or_reply(event, "**ꪎ︙جاري الغاء حظر العام .... **")
    start = datetime.now()
    user, reason = await get_user_from_event(event, cate)
    if not user:
        return
...
@matrix.on(admin_cmd(pattern=r"معلومات تخزين المجموعه(?:\s|$)([\s\S]*)"))
async def _(event):  # sourcery no-metrics
    reply = await event.get_reply_message()
    input_str = event.pattern_match.group(1)
    ...
    try:
        ...
        return await edit_delete(
            event, f"<b>ꪎ︙ خطـأ ⚠️ : </b><code>{str(e)}</code>", 5, parse_mode="HTML"
        )
    try:
        userdata = await event.client.get_entity(userentity)
    except Exception as e:
        return await edit_delete(
            event, f"<b>ꪎ︙ خطـأ ⚠️ : </b><code>{str(e)}</code>", time=5, parse_mode="HTML"
        )
    ...
    catevent = await edit_or_reply(
        event,
        f"<code>ꪎ︙ حسـاب عـدد الملفـات وحجـم الملـف حسـب ✦ </code>{_format.htmlmentionuser(userdata.first_name,userdata.id)}<code> in Group </code><b>{link}</b>\n<code>This may take some time also depends on number of user messages</code>",
        parse_mode="HTML",
    )
    ...
    totalstring = f"<code><b> ꪎ︙ إجمالـي الملفـات ✦ : </b>       | {str(totalcount)}\
                  \n <b> ꪎ︙ الحجـم الإجمالـي للملـف ✦ : </b>   | {humanbytes(totalsize)}\
                  \n <b> حجم الملف  : </b>    | {avghubytes}\
                  \n</code>"
    runtimestring = f"<code><b> ꪎ︙ وقـت التشغيـل ✦ :</b>            | {runtime}\
                    \n <b> وقـت التشغيـل لڪل ملـف ✦ :</b>   | {avgruntime}\
                    \n</code>"
    line = "<code>+--------------------+-----------+</code>\n"
    result = f"<b>ꪎ︙ المجموعـة ✦ : {link}\nUser : {_format.htmlmentionuser(userdata.first_name,userdata.id)}\n\n"
    result += f"<code><b>ꪎ︙ مجمـوع الرسائـل ✦ :</b> {msg_count}</code>\n"
    result += "<b>ꪎ︙ ملخـص الملـف ✦ : </b>\n"
                    ...
                    if LOG_CHATS_.COUNT > 1:
                        await LOG_CHATS_.NEWPM.edit(
                            LOG_CHATS_.NEWPM.text.replace(
                                "ꪎ︙  رسـالة جـديدة", f"{LOG_CHATS_.COUNT} "
                            )
                        )
                    else:
                        await LOG_CHATS_.NEWPM.edit(
...
@matrix.iq_cmd(incoming=True, func=lambda e: e.mentioned, edited=False, forword=None)
async def log_tagged_messages(event):
    ...
    resalt = f"ꪎ︙  المجموعه : </b><code>{hmm.title}</code>"
    if full is not None:
        resalt += (
            f"\n<b>ꪎ︙  من : </b> 👤{_format.htmlmentionuser(full.first_name , full.id)}"
        )
    if messaget is not None:
        resalt += f"\n<b>ꪎ︙  رسـالة جـديدة : </b><code>{messaget}</code>"
    else:
        resalt += f"\n<b>ꪎ︙  رسـالة جـديدة: </b>{event.message.message}"
    resalt += f"\n<b>ꪎ︙  رابط الرساله : </b><a href = 'https://t.me/c/{hmm.id}/{event.message.id}'> اضغط هنا</a>"
...
@matrix.on(admin_cmd(pattern=r"تخزين الخاص (تشغيل|ايقاف)$"))
async def set_pmlog(event):
    ...
    if PMLOG:
        if h_type:
            await event.edit("**ꪎ︙  تـخزين رسـائل الخـاص بالفـعل مُمكـنة ✅**")
        else:
            addgvar("PMLOG", h_type)
            await event.edit("**ꪎ︙  تـم تعـطيل تخـزين رسائل الـخاص بنـجاح ✅**")
    elif h_type:
        addgvar("PMLOG", h_type)
        await event.edit("**ꪎ︙  تـم تفعيل تخـزين رسائل الـخاص بنـجاح ✅**")
    else:
        await event.edit("**ꪎ︙  تـخزين رسـائل الخـاص بالفـعل معـطلة ✅**")

@matrix.on(admin_cmd(pattern=r"تخزين الكروبات (تشغيل|ايقاف)$"))
async def set_grplog(event):
    ...
    if GRPLOG:
        if h_type:
            await event.edit("**ꪎ︙  تـخزين رسـائل الكروبات بالفـعل مُمكـنة ✅**")
        else:
            addgvar("GRPLOG", h_type)
            await event.edit("**ꪎ︙  تـم تعـطيل تخـزين رسائل الكروبات بنـجاح ✅**")
    elif h_type:
        addgvar("GRPLOG", h_type)
        await event.edit("**ꪎ︙  تـم تفعيل تخـزين رسائل الكروبات بنـجاح ✅**")
    else:
        await event.edit("**ꪎ︙  تـخزين رسـائل الكروبات بالفـعل معـطلة ✅**")    
    
@matrix.on(admin_cmd(pattern=f"{LINKK} ?(.*)"))
async def iq(SLQ):
...
@matrix.on(admin_cmd(pattern="عدد رسائلي ?(.*)"))
async def iq(SLQ):
...
@matrix.on(admin_cmd(pattern="تغير صورة( المجموعة| -d)$"))
async def set_group_photo(event):  # sourcery no-metrics
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            "ꪎ︙  صوره_المجموعة\n"
            f"ꪎ︙  صورة المجموعه {process} بنجاح "
            f"ꪎ︙  المحادثة  📜 : {event.chat.title}(`{event.chat_id}`)",
        )
...
@matrix.on(admin_cmd(pattern=f"{ADMINRAISE}(?: |$)(.*)"))
async def promote(event):
    ...
    if not user:
        return
    catevent = await edit_or_reply(event, "**ꪎ︙ يـتم الرفـع  ↗️ **")
    try:
        await event.client(EditAdminRequest(event.chat_id, user.id, new_rights, rank))
    except BadRequestError:
        return await catevent.edit(NO_PERM)
    await catevent.edit("**ꪎ︙ تم رفعه مشرف بالمجموعه بنجاح  📤**")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"ꪎ︙ ترقية  🆙\
            \nꪎ︙ المستخدم  🚹 : [{user.first_name}](tg://user?id={user.id})\
            \nꪎ︙ المحادثة  📜 : {event.chat.title} (`{event.chat_id}`)",
        )

@matrix.on(admin_cmd(pattern=f"{UNADMINRAISE}(?: |$)(.*)"))
async def demote(event):
    "لتنزيل من رتبة الادمن"
    user, _ = await get_user_from_event(event)
    if not user:
        return
    catevent = await edit_or_reply(event, "**ꪎ︙ يـتم التنزيل من الاشراف  ↙️**")
    ...
    try:
        ...
        return await catevent.edit(NO_PERM)
    await catevent.edit("**ꪎ︙ تـم تنزيله من قائمه الادمنيه بنجاح  ✅**")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"ꪎ︙  تنزيل_مشرف\
...
@matrix.on(admin_cmd(pattern="تثبيت(?: |$)(.*)"))
async def pin(event):
    "ꪎ︙  تثبيت  📌"
    to_pin = event.reply_to_msg_id
    if not to_pin:
        return await edit_delete(event, "**ꪎ︙ يرجى الرد على الرسالة التي تريد تثبيتها 📨 **", 5)
    ...
    await edit_delete(event, "**ꪎ︙ تم تثبيت الرسالة بنجاح في هذه الدردشة  📌**", 3)
    if BOTLOG and not event.is_private:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"ꪎ︙  تثبيت  📌\
                \nꪎ︙  تم تثبيت الرسالة بنجاح في الدردشة  📌\
                \nꪎ︙ المستخدم  🚹 : {event.chat.title}(`{event.chat_id}`)\
                \nꪎ︙ المحادثة  📜 : {is_silent}",
        )


@matrix.on(admin_cmd(pattern="الغاء التثبيت(?: |$)(.*)"))
async def pin(event):
    "ꪎ︙ لإلغاء تثبيت رسائل من المجموعة  ⚠️"
    to_unpin = event.reply_to_msg_id
    options = (event.pattern_match.group(1)).strip()
    if not to_unpin and options != "all":
        return await edit_delete(
            event,
            "ꪎ︙  يرجى الرد على الرسالة التي تريد تثبيتها استخدم `.الغاء التثبيت للكل`  لالغاء تثبيت جميع الرسائل  📍",
            5,
        )
    try:
        if to_unpin and not options:
            ...
            return await edit_delete(
                event, "ꪎ︙  يرجى الرد على الرسالة التي تريد تثبيتها استخدم `.الغاء التثبيت للكل`  لالغاء تثبيت جميع الرسائل  📍", 5
            )
    except BadRequestError:
        return await edit_delete(event, NO_PERM, 5)
    except Exception as e:
        return await edit_delete(event, f"`{str(e)}`", 5)
    await edit_delete(event, "**ꪎ︙ تم الغاء التثبيت بنجاح  ✅**", 3)
    if BOTLOG and not event.is_private:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"**ꪎ︙  الـغاء التثبيت  ❗️ \
                \n** ꪎ︙  تم بنجاح الغاء التثبيـت في الدردشة  ✅ \
                \nꪎ︙ الدردشـه  🔖 : {event.chat.title}(`{event.chat_id}`)",
        )

@matrix.on(admin_cmd(pattern="جلب الاحداث(?: |$)(.*)"))
async def _iundlt(event):  # sourcery no-metrics
    "ꪎ︙ لأخذ نظرة عن آخر الرسائل المحذوفة في المجموعة  💠"
    catevent = await edit_or_reply(event, "**ꪎ︙ يتم البحث عن اخر الاحداث انتظر  🔍**")
    ...
    deleted_msg = f"**ꪎ︙  اخر {lim} رسائل محذوفة في هذه المجموعة  🗑 :**"
    if not flag:
        for msg in adminlog:
            ruser = (
...
moment_worker = []
@matrix.on(admin_cmd(outgoing=True, pattern="all?(.*)"))
async def mentionall(event):
  ...
  if mode == "text_on_cmd":
    ...
    usrnum = 0
    usrtxt = ""
    async for usr in matrix.iter_participants(event.chat_id):
      usrnum += 1
      ...
      if event.chat_id not in moment_worker:
        ...
        return
      if usrnum == 5:
        await matrix.send_message(event.chat_id, f"{usrtxt}\n\n{msg}")
  ...
  if mode == "text_on_reply":
    ...
    async for usr in matrix.iter_participants(event.chat_id):
      ...
      if event.chat_id not in moment_worker:
        ...
        return
      if usrnum == 5:
        await matrix.send_message(event.chat_id, usrtxt, reply_to=msg)
        await asyncio.sleep(2)
        usrnum = 0
        usrtxt = ""

@matrix.on(admin_cmd(outgoing=True, pattern="ايقاف التاك?(.*)"))
...
@matrix.on(admin_cmd(pattern=f"{BANDD}(?: |$)(.*)"))
async def _ban_person(event):
    "ꪎ︙  لحـظر شخص في كـروب مـعين"
    user, reason = await get_user_from_event(event)
    if not user:
        return
    if user.id == 1226408155:
        return await edit_delete(event, "**ꪎ︙  عـذرا أنـة مبـرمج السـورس  ⚜️**")
    if user.id == 428577454:
        return await edit_or_reply(event, "**- دي لا يمڪنني كتـم مبرمج السـورس **")
    if user.id == event.client.uid:
        return await edit_delete(event, "ꪎ︙  عـذرا لا تسـتطيع حـظر شـخص")
    catevent = await edit_or_reply(event, "ꪎ︙  تـم حـظره بـنجاح")
    try:
    ...
    try:
        ...
        return await catevent.edit(
            "ꪎ︙  ليـس لـدي جـميع الصـلاحيـات لكـن سيـبقى محـظور"
        )
    if reason:
        await catevent.edit(
            f"ꪎ︙  المسـتخدم {_format.mentionuser(user.first_name ,user.id)} \n ꪎ︙  تـم حـظره بنـجاح !!\n**ꪎ︙ السبب : **`{reason}`"
        )
    else:
        await catevent.edit(
            f"ꪎ︙  المسـتخدم {_format.mentionuser(user.first_name ,user.id)} \n ꪎ︙  تـم حـظره بنـجاح ✅"
...
@matrix.on(admin_cmd(pattern=r"zip(?:\s|$)([\s\S]*)"))
...
@matrix.on(admin_cmd(pattern=r"unzip(?:\s|$)([\s\S]*)"))
...
@matrix.on(admin_cmd(pattern="معرفه(?: |$)(.*)"))
...
@matrix.on(admin_cmd(pattern="دعوه للمكالمه(?: |$)(.*)"))
async def _(e):
...
# MULTE CHAT
@matrix.on(events.NewMessage(outgoing=True, pattern='.كتم الدردشه'))
async def GlobalMulte(event):
    chat = await event.get_chat()
    
    if chat.id not in ON_WATCH_KNOMUTE:
        ON_WATCH_KNOMUTE.append(chat.id)
        if chat.id not in ADMINSMUTEKNO:
            ADMINSMUTEKNO[chat.id] = {}
        
        ADMINSMUTEKNO = await event.client(GetParticipantsRequest(channel=chat.id, filter=ChannelParticipantsADMINSMUTEKNO(), offset=0, limit=0, hash=0))
        ADMINSMUTEKNO_LIST = ""
        for admin in ADMINSMUTEKNO.users:
            ADMINSMUTEKNO_LIST = ADMINSMUTEKNO_LIST + str(admin.id) +","
            
        ADMINSMUTEKNO[chat.id] = ADMINSMUTEKNO_LIST
        
    order = await event.edit("**تم تفعيل كتم الدردشه**")

# UNMULTE CHAT
@matrix.on(events.NewMessage(outgoing=True, pattern='.اغلاق كتم الدردشه'))
async def GlobalUNMulte(event):
    chat = await event.get_chat()
    ME = await matrix.get_me()
        
    if chat.id in CHANNELSMUTEKNO:  
        ADMINSMUTEKNO = []
        ADMIN_ID = ADMINSMUTEKNO[chat.id].split(",")
        for admin_id in ADMIN_ID:
            if admin_id != "":
                ADMINSMUTEKNO.append(int(admin_id))
                    
        for admin in ADMINSMUTEKNO:
            if admin != ME.id:
                try:
                    from telethon.tl.types import ChatAdminRights
                    edit = await event.client(EditAdminRequest(channel=chat.id, user_id=admin, admin_rights=ChatAdminRights(post_messages=True, change_info=True), rank=''))
                except Exception as e:
                    print (e)
                    
    if chat.id in ON_WATCH_KNOMUTE:
        ON_WATCH_KNOMUTE.remove(chat.id)
    
    if chat.id in ADMINSMUTEKNO:
        ADMINSMUTEKNO.pop(chat.id)


# WATCH NEW MESSAGES
@matrix.on(events.NewMessage)
async def GlobalWatch(event):
    chat = await event.get_chat()
    sender = await event.get_sender()
    ME = await matrix.get_me()
    ...
    if chat.id in ON_WATCH_KNOMUTE:
        ...
        if event.is_group == True:        
            if CHAT.megagroup == True:
                # GROUP
                if sender.id not in ADMINSMUTEKNO:
                    delete = await event.client(DeleteMessagesRequest(channel=chat.id, id=[event.message.id]))
            ...
            for admin in ADMINSMUTEKNO:
                if admin != ME.id:
                    try:
                        from telethon.tl.types import ChatAdminRights
                        edit = await event.client(EditAdminRequest(channel=chat.id, user_id=admin, admin_rights=ChatAdminRights(post_messages=None, change_info=True), rank=''))
                        CHANNELSMUTEKNO.append(chat.id)
                    except Exception as e:
                        print (e)
@matrix.on(admin_cmd(pattern="بدء مكالمه(?: |$)(.*)"))
async def _(e):
    try:
...
@matrix.on(admin_cmd(pattern=f"{UNBANDD}(?: |$)(.*)"))
async def nothanos(event):
    "ꪎ︙  لألـغاء الـحظر لـشخص في كـروب مـعين"
    user, _ = await get_user_from_event(event)
    if not user:
        return
    catevent = await edit_or_reply(event, "ꪎ︙  جـار الـغاء الـحظر أنتـظر")
    try:
        await event.client(EditBannedRequest(event.chat_id, user.id, UNBAN_RIGHTS))
        await catevent.edit(
            f"ꪎ︙  الـمستخدم {_format.mentionuser(user.first_name ,user.id)}\n ꪎ︙  تـم الـغاء حـظره بنـجاح "
        )
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
        ...
        await catevent.edit("ꪎ︙  يـبدو أن هذه الـعمليـة تم إلغاؤهـا")
    except Exception as e:
...
async def watcher(event):
    if is_muted(event.sender_id, event.chat_id):
        try:
            ...
            LOGS.info(str(e))
@matrix.on(admin_cmd(pattern="صنع (مجموعه|قناه) (.*)"))
async def iq(event):
    type_of_group = event.pattern_match.group(1)
    group_name = event.pattern_match.group(2)
    if type_of_group == "قناه":
        descript = "ꪎ︙  هذه قناة إختبار أُنشئت بإستعمال تليثون العرب"
    else:
        descript = "ꪎ︙  هذه المجموعه إختبار أُنشئت بإستعمال تليثون العرب"
    if type_of_group == "مجموعه":
        try:
            ...
            await edit_or_reply(event, f"ꪎ︙  اسم المجموعه `{group_name}` ** تم الإنشاء بنجاح  ✅  دخول ** {result.link}")
        except Exception as e:
            await edit_delete(event, f"**ꪎ︙  حدث خطأ ما  🆘:**\n{str(e)}")
        ...
        try:
            ...
            await edit_or_reply(event, f"ꪎ︙  اسم القناه `{group_name}` ** تم الإنشاء بنجاح  ✅  دخول ** {result.link}")
        except Exception as e:
            await edit_delete(event, f"**ꪎ︙  حدث خطأ ما  🆘 :**\n{str(e)}")
    elif type_of_group == "مجموعه":
        answer = await create_supergroup(group_name, event.client, Config.TG_BOT_USERNAME, descript)
        if answer[0] != "error":
            await edit_or_reply(event, f"ꪎ︙  خارق جروب `{group_name}` ** تم الإنشاء بنجاح  ✅  دخول ** {answer[0].link}")
        else:
            await edit_delete(event, f"**ꪎ︙  حدث خطأ ما  🆘 :**\n{str(answer[1])}")
    else:
        await edit_delete(event, "**ꪎ︙ الاوامر` **صنع مجموعه لمعرفة كيفية استخدامي.`")
@matrix.on(admin_cmd(pattern="تلقائي ?(.*)"))
async def _(event):
...
@matrix.on(admin_cmd(pattern="الغاء_التلقائي ?(.*)"))
async def _(event):
    if (event.is_private or event.is_group):
        return await edit_or_reply(event, "امر التلقائي يستخدم فقط للقنوات.")
    hel_ = event.pattern_match.group(1)
...
@matrix.on(admin_cmd(pattern=r"جلب الصور(?:\s|$)([\s\S]*)"))
async def potocmd(event):
    ...
    if uid.strip() == "":
        ...
        if int(uid) > (len(photos)):
            return await edit_delete(
                event, "**ꪎ︙  لم يتم العثور على صورة لهذا  الشخص 🏞**"
            )
        ...
        try:
            ...
            if uid <= 0:
                await edit_or_reply(
                    event, "**ꪎ︙  الرقم غير صحيح - اختر رقم صوره موجود فعليا ⁉️**"
                )
                return
        except BaseException:
        ...
        await event.client.send_file(event.chat_id, send_photos)
    await event.delete()
@matrix.on(admin_cmd(pattern="تفعيل ([\s\S]*)"))    
async def _(event):  
    input_str = event.pattern_match.group(1)
    peer_id = event.chat_id
    if not event.is_group:
        return await edit_delete(event, "**ꪎ︙هذه ليست مجموعة لقفل الأشياء**")
    ...
    if input_str in (("bots", "commands", "email", "forward", "url")):
        ...
        if input_str == "msg":
            if msg:
                return await edit_delete(event, "**ꪎ︙هذه المجموعة مؤمنة بالفعل بإذن المراسلة**")
            msg = True
            locktype = "messages"
            ...
            return await edit_delete(event, f"**ꪎ︙عذرا خطا بكتابه الأمر :** `{input_str}`", time=5)

        else:
            return await edit_or_reply(event, "**ꪎ︙لااستطيع تفعيل حمايه المجموعه**")
        try:
            cat = Get(cat)
        ...
        try:
            ...
            await edit_or_reply(event, f"**ꪎ︙تفعيل حمايه المجموعه تم بنجاح**")
        except BaseException as e:
            await edit_delete(event,f"**ꪎ︙هناك خطا:** `{e}`", time=5)
@matrix.on(admin_cmd(pattern="تعطيل ([\s\S]*)"))    
async def _(event):  
    input_str = event.pattern_match.group(1)
    peer_id = event.chat_id
    if not event.is_group:
    ...
    if input_str in (("bots", "commands", "email", "forward", "url")):
        ...
        await edit_or_reply(event, "**ꪎ︙تعطيل حمايه المجموعه تم بنجاح**".format(input_str))
    else:
        msg = chat_per.send_messages
        media = chat_per.send_media
        sticker = chat_per.send_stickers
        ...
        if input_str == "msg":
            if not msg:
                return await edit_delete(event, "**ꪎ︙هذه المجموعة غير مؤمنة بالفعل بإذن المراسلة**")
            msg = False
            locktype = "messages"
        elif input_str == "حمايه المجموعه":
            msg = False
            ...
            return await edit_or_reply(event, "**ꪎ︙لااستطيع تعطيل حمايه المجموعه**")
        try:
            cat = Get(cat)
            await event.client(cat)
        except BaseException:
        ...
        try:
            ...
            await edit_or_reply(event, "**ꪎ︙تعطيل حمايه المجموعه تم بنجاح**")
        except BaseException as e:
            return await edit_delete(event, f"**ꪎ︙هناك خطا:** `{e}`", time=5)
@matrix.on(admin_cmd(pattern="صلاحيات المجموعه$"))    
async def _(event):  
    res = ""
    ...
    if not current_db_locks:
        ...
        res = "**ꪎ︙أذونات الصلاحيه في هذه الدردشة : **\n"
        ...
        res += f"**ꪎ︙البوتات :** `{ubots}`\n"
        res += f"**ꪎ︙الرسائل :** `{ucommands}`\n"
        res += f"**ꪎ︙التوجيهات :** `{uforward}`\n"
        res += f"**ꪎ︙الروابط :** `{uurl}`\n"
    ...
    try:
        ...
        res += "\n**ꪎ︙هذه هي الأذونات الحالية لهذه الدردشة :** \n"
        res += f"**ꪎ︙الرسائل :** `{umsg}`\n"
        res += f"**ꪎ︙الميديا :** `{umedia}`\n"
        res += f"**?? ⦙ الملصقات :** `{usticker}`\n"
        res += f"**ꪎ︙المتحركه :** `{ugif}`\n"
        res += f"**ꪎ︙معاينه الروابط :** `{uembed_link}`\n"
        res += f"**ꪎ︙الالعاب :** `{ugamee}`\n"
        res += f"**ꪎ︙الاونلاين :** `{uainline}`\n"
        res += f"**ꪎ︙اضافه الاعضاء :** `{uadduser}`\n"
        res += f"**ꪎ︙تغير معلومات :** `{uchangeinfo}`\n"
    await edit_or_reply(event, res)
@matrix.on(events.ChatAction())
async def _(event):
    if not event.is_private:
        ...
        for user_id in added_users:
            ...
            if user_obj.bot:
                ...
                try:
                    ...
                    await event.reply("**ꪎ︙لا يبدو أن لدي صلاحيه هنا. **\n`{}`".format(str(e)))
                    update_lock(event.chat_id, "bots", False)
                    break
        if BOTLOG and is_ban_able:
            ban_reason_msg = await event.reply("**ꪎ︙تحذير [user](tg://user?id={}) من فضلك لا تضيف الروبوتات إلى هذه الدردشة.**".format(users_added_by))