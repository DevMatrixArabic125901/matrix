    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.COMMAND_HAND_LER) == 2:
                matrixreg = f"^{Config.COMMAND_HAND_LER}"
                reg = Config.COMMAND_HAND_LER[1]
            elif len(Config.COMMAND_HAND_LER) == 1:
                matrixreg = f"^\\{Config.COMMAND_HAND_LER}"
                reg = Config.COMMAND_HAND_LER
            args["pattern"] = re.compile(matrixreg + pattern)
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_COMMAND_HAND_LER) == 2:
                matrixreg = f"^{Config.SUDO_COMMAND_HAND_LER}"
...
def register(**args):
    ...
    def decorator(func):
        if not disable_edited:
            matrix.add_event_handler(func, MessageEdited(**args))
        matrix.add_event_handler(func, NewMessage(**args))
        try:
