...
LOGS = logging.getLogger("——————— Matrix Arabic")
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["matrix.plugins." + shortname] = mod
        LOGS.info(f"[تم تحميل : {shortname}] \n ——————— Matrix Arabic")
...
def remove_plugin(shortname):
    ...
    try:
        name = f"matrix.plugins.{shortname}"
        for i in reversed(range(len(matrix._event_builders))):
            ev, cb = matrix._event_builders[i]
            if cb.__module__ == name:
                del matrix._event_builders[i]
    except BaseException:
        raise ValueError
...
def checkplugins(filename):
    ...
    with open(filename, "w") as matrix:
        matrix.write(filedata)