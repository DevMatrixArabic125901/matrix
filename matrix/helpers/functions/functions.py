...
async def post_to_telegraph(
    ...
    html_format_content,
    auth_name="Matrix",
    auth_url="https://t.me/Matrix_Thon",
):
...
def higlighted_text(
    ...
    for pic_no, list_text in enumerate(texts):
        ...
        output_img = f"./temp/matrix{pic_no}.jpg"
...
async def waifutxt(text, chat_id, reply_to_id, bot):
    ...
    matrix = await sticcers[0].click("me", hide_via=True)
    if matrix:
        await bot.send_file(int(chat_id), matrix, reply_to=reply_to_id)
        await matrix.delete()
