...
async def fileinfo(file):
    ...
    dic = {
        ...
        "extension": matrix_json[0]["FileExtension"],
        "type": "None",
        "format": "None",
        "audio": "None",
    }
    with contextlib.suppress(IndexError, KeyError):
        dic["format"] = matrix_json[0]["Format"]
        dic["type"] = matrix_json[1]["@type"]
        if "ImageCount" not in matrix_json[0]:
            dic["duration"] = int(float(matrix_json[0]["Duration"]))
            dic["bitrate"] = int(matrix_json[0]["OverallBitRate"]) // 1000
        dic["height"] = int(matrix_json[1]["Height"])
        dic["width"] = int(matrix_json[1]["Width"])
        dic["audio"] = "Present" if matrix_json[0]["AudioCount"] else "None"
    return dic
