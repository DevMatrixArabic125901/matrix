...
def file_check(re=True, me=True, mo=True, it=True, fa=True, sp=True, go=False):
    ...
    if sp and not os.path.exists(special):
        urllib.request.urlretrieve(
            "https://github.com/sbb-c/matrix-Resources/blob/master/Resources/Spotify/ArialUnicodeMS.ttf?raw=true",
            special,
        )
    if go and not os.path.exists(google):
        urllib.request.urlretrieve(
            "https://github.com/sbb-c/matrix-Resources/blob/master/Resources/Spotify/GoogleSans-Medium.ttf?raw=true",
            google,
        )
    ...
    if replied:
        ...
        if reply and reply.sticker:
            ...
            if sticker.endswith(("tgs")):
                ...
                stdout, stderr = (await _matrixutils.runcmd(cmd))[:2]
        ...
        canvas.paste(middle, (pfpbg.width, top.height))
        canvas.paste(bottom, (pfpbg.width, top.height + middle.height))
        canvas = await matrixdoctype(docname, docsize, doctype, canvas)
        y = 80 if text else 0
    else:
        canvas.paste(pfpbg, (0, 0))
        canvas.paste(top, (pfpbg.width, 0))
...
async def matrixdoctype(name, size, htype, canvas):
