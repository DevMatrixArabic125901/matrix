...
CATLOGO = "https://graph.org/file/214d304f80ff761073560.jpg"
...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**matrix"):
            buttons = [
                (
                    ...
                    Button.url("DEV", "https://t.me/Matrix_Thon"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./matrix", "troll.txt")
            try:
            ...
            try:
                ...
                try:
                    ...
                    if u.username:
                        MATRIXVX = f"@{u.username}"
                    else:
                        MATRIXVX = f"[{u.first_name}](tg://user?id={u.id})"
                    u = int(u.id)
                except ValueError:
                    MATRIXVX = f"[user](tg://user?id={u})"
            except ValueError:
                # if u is username
                try:
            ...
            result = builder.article(
                ...
                text=f"🔒 هذه همسة سرية الى {MATRIXVX}, هو فقط من يستطيع رؤيتها",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./matrix", "hide.txt")
            try:
            ...
            result = builder.article(
                title="MATRIX Help™",
                description="**✾ قائمة التعليمات الخاصة ب ماتركس **",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قناة السورس", "t.me/Matrix_Thon"),
                Button.url(
                    "المطور",
                    "t.me/FFlXlX",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "لجعل ماتركس من نصيبك!", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="[𝙈𝘼𝙏𝙍𝙄𝙓 𝙐𝙎𝙀𝙍𝘽𝙊𝙏](t.me/Matrix_Thon)",
            description="لتنصيبه لك",
            url="t.me/Matrix_Thon",
            thumb=photo,
            content=photo,
            send_message=types.InputBotInlineMessageMediaAuto(
                reply_markup=markup, message=text, entities=msg_entities
...
@matrix.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@matrix.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@matrix.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@matrix.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
