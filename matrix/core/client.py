...
class MatrixClient(TelegramClient):
    def ar_cmd(
        self: TelegramClient,
        ...
        **kwargs,
        ...
        def decorator(func):  # sourcery no-metrics
            async def wrapper(check):  # sourcery no-metrics
                ...
                if hasattr(chat, "title"):
                    if (
                        ...
                        await check.client.send_message(
                            "@Matrix_Thon", "اهلا محمد استخدامي ما يناسب ماتركس"
                        )
                        return
                ...
                try:
                    ...
                    if not disable_errors:
                        ...
                        link = "[هنا](https://t.me/MatrixzSupport)"
                        text = "**تقرير خطأ سورس ماتركس**\n\n" + "اذا اردت يمكنك التبليغ"
            ...
            if pattern is not None:
                ...
                if edited:
                    matrix.add_event_handler(func, events.MessageEdited(**kwargs))
                matrix.add_event_handler(func, events.NewMessage(**kwargs))
            return wrapper
    ...
    def bot_cmd(
        ...
        **kwargs,
        ...
        def decorator(func):
            ...
            if edited is True:
                matrix.tgbot.add_event_handler(func, events.MessageEdited(**kwargs))
            else:
                matrix.tgbot.add_event_handler(func, events.NewMessage(**kwargs))
...
MatrixClient.fast_download_file = download_file
MatrixClient.fast_upload_file = upload_file
MatrixClient.reload = restart_script
MatrixClient.get_msg_link = get_message_link
MatrixClient.check_testcases = checking
try:
    send_message_check = TelegramClient.send_message
except AttributeError:
    MatrixClient.send_message = send_message
    MatrixClient.send_file = send_file
    MatrixClient.edit_message = edit_message
